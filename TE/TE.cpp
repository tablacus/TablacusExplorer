...
VOID teUnadvise(IUnknown *punk, IID diid, PDWORD pdwCookie)
{
	IConnectionPointContainer *pCPC;
	if SUCCEEDED(punk->QueryInterface(IID_PPV_ARGS(&pCPC))) {
		IConnectionPoint *pCP;
...
VOID CALLBACK teEnsureVisibleProc(HWND hwnd, UINT uMsg, UINT_PTR idEvent, DWORD dwTime)
{
	KillTimer(hwnd, idEvent);
	((CteShellBrowser *)idEvent)->EnsureFocusedItem();
}
			...
			if (pSB) {
				...
				if (pSB->m_pShellView && SUCCEEDED(pSB->m_pShellView->QueryInterface(IID_PPV_ARGS(&pFV)))) {
					...
					if SUCCEEDED(pFV->ItemCount(SVGIO_SELECTION, &nCount)) {
						...
						BSTR bs = ::SysAllocStringLen(L"", 100);
						LoadString(g_hShell32, uID, bs, 100);
						bs[4] = _T('%');
						bs[5] = _T('d');
						swprintf_s(&bs[50], 50, &bs[4], nCount);
						DoStatusText(pSB, &bs[50], 0);
						SysFreeString(bs);
					}
					pFV->Release();
				}
					...
					int       nCmdShow)
{
	...
	try {
		teUnadvise(pSW, DIID_DShellWindowsEvents, &dwSWCookie);
	} catch (...) {}
	try {
...
CteShellBrowser::~CteShellBrowser()
{
	...
	if (m_pDSFV) {
		teUnadvise(m_pDSFV, DIID_DShellFolderViewEvents, &m_dwCookie);
		m_pDSFV->Release();
	}
#ifdef _2000XP
	if (m_pSFVCB) {
		...
		if SUCCEEDED(m_pShellView->QueryInterface(IID_PPV_ARGS(&pFV))) {
			...
			if SUCCEEDED(pFV->GetFocusedItem(&i)) {
				...
				if SUCCEEDED(pFV->Item(i, &pidl)) {
					...
					teSysFreeString(&pid1->m_bsFocus);
					GetDisplayNameFromPidl(&pid1->m_bsFocus, pidl, SHGDN_INFOLDER | SHGDN_FORPARSING);
					pid1->Release();
					teCoTaskMemFree(pidl);
				}
			}
...
VOID CteShellBrowser::EnsureFocusedItem()
{
	if (m_pSF2) {
		CteFolderItem *pid;
		if SUCCEEDED(m_pFolderItem->QueryInterface(g_ClsIdFI, (LPVOID *)&pid)) {
			if (pid->m_bsFocus) {
				LPITEMIDLIST pidl;
				if SUCCEEDED(m_pSF2->ParseDisplayName(g_hwndMain, NULL, pid->m_bsFocus, NULL, &pidl, NULL)) {
					m_pShellView->SelectItem(ILFindLastID(pidl), SVSI_FOCUSED | SVSI_ENSUREVISIBLE | SVSI_NOTAKEFOCUS);
					teCoTaskMemFree(pidl);
				}
				::teSysFreeString(&pid->m_bsFocus);
			}
			pid->Release();
		}
	}
	...
	try {
		...
		switch(dispIdMember) {
			...
			case DISPID_FILELISTENUMDONE:
				...
				return S_OK;
			case DISPID_FOCUSCHANGED:
				EnsureFocusedItem();
				return S_OK;
			case DISPID_VIEWMODECHANGED:
				PostMessage(m_hwndLV, LVM_SETEXTENDEDLISTVIEWSTYLE, LVS_EX_FULLROWSELECT, (m_param[SB_FolderFlags] & FWF_FULLROWSELECT) ? LVS_EX_FULLROWSELECT : 0);
				return S_OK;
			default:
...
STDMETHODIMP CteShellBrowser::OnNavigationComplete(PCIDLIST_ABSOLUTE pidlFolder)
{
	...
	KillTimer(m_hwnd, (UINT_PTR)this);
	SetTimer(m_hwnd, (UINT_PTR)this, 32, teEnsureVisibleProc);
	return S_OK;
}
...
#ifdef _2000XP
					if (!pPreviousView && osInfo.dwMajorVersion == 5) {
					}
#endif
#ifdef _VISTA7
					if (hr == S_FALSE) {
...
void CteWebBrowser::Close()
{
	if (m_pWebBrowser) {
		m_pWebBrowser->Quit();
		teUnadvise(m_pWebBrowser, DIID_DWebBrowserEvents2, &m_dwCookie);
		IOleObject *pOleObject;
		if SUCCEEDED(m_pWebBrowser->QueryInterface(IID_PPV_ARGS(&pOleObject))) {
			RECT rc;
			SetRectEmpty(&rc);
			...
			case 2470:
				if (pVarResult && nArg >= 0) {
					...
					if (teILGetParent(pFolderItem, &pFolderItem1)) {
						teSetObject(pVarResult, pFolderItem1);
