...
BOOL tePathIsNetworkPath(LPCWSTR pszPath)//PathIsNetworkPath is slow.
{
	...
	return (pszDrive[0] == '\\' && pszDrive[1] == '\\') || GetDriveType(pszDrive) == DRIVE_REMOTE;
}

VOID teCommaSize(LPWSTR pszBuf, UINT cchBuf)
{
	int i = 0;
	int j = 0;
...
VOID teStrFormatSize(DWORD dwFormat, LONGLONG qdw, LPWSTR pszBuf, UINT cchBuf)
{
	...
	if (dwFormat == 3) {
		swprintf_s(pszBuf, cchBuf, L"%llu", qdw);
		teCommaSize(pszBuf, cchBuf);
		return;
	}
	LPWSTR pszPrefix = L"\0KMGTPE\0";
	WCHAR pszFormat[MAX_COLUMN_NAME_LEN];
	int i = (dwFormat >> 4) & 7;
	swprintf_s(pszFormat, MAX_COLUMN_NAME_LEN, L"%%.%df %cB", dwFormat & 0xf, pszPrefix[i]);
	LONGLONG llPow = 1;
	if (i < 7) {
		while (i-- > 0) {
			llPow *= 1024;
	...
	swprintf_s(pszBuf, cchBuf, pszFormat, double(qdw) / llPow);
	teCommaSize(pszBuf, cchBuf);
}
...
BSTR tePSGetNameFromPropertyKeyEx(PROPERTYKEY propKey, int nFormat)
{
	if (nFormat == 2) {
		WCHAR szProp[64];
...
ULONGLONG teGetFolderSize(LPCWSTR szPath, int nLevel, CteShellBrowser *pSB, LPITEMIDLIST pidl)
{
	if (pSB->m_pidl != pidl) {
		return 0;
	}
		...
		do {
			...
			if (wfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
				if (nLevel) {
					...
					Result += teGetFolderSize(bs, nLevel - 1, pSB, pidl);
				}
				continue;
			}
			ULARGE_INTEGER uli;
...
LPITEMIDLIST teILCreateFromPath2(IShellFolder *pSF, LPWSTR pszPath)
{
	...
	if SUCCEEDED(pSF->EnumObjects(NULL, SHCONTF_FOLDERS | SHCONTF_NONFOLDERS | SHCONTF_INCLUDEHIDDEN | SHCONTF_INCLUDESUPERHIDDEN, &peidl)) {
		LPITEMIDLIST pidlPart;
		ULONG celtFetched;
		while (!pidlResult && peidl->Next(1, &pidlPart, &celtFetched) == S_OK) {
			for (int j = 0; j < 2; j++) {
					...
					if (nDelimiter) {
						if (nDelimiter == ::SysStringLen(bstr) && StrCmpNI(const_cast<LPCWSTR>(pszPath), (LPCWSTR)bstr, nDelimiter) == 0) {
							...
							if SUCCEEDED(pSF->BindToObject(pidlPart, NULL, IID_PPV_ARGS(&pSF1))) {
								pidlResult = teILCreateFromPath2(pSF1, &lpDelimiter[1]);
								pSF1->Release();
							}
							::SysFreeString(bstr);
							break;
...
HRESULT GetDisplayNameFromPidl(BSTR *pbs, LPITEMIDLIST pidl, SHGDNF uFlags)
{
	...
	if SUCCEEDED(SHBindToParent(pidl, IID_PPV_ARGS(&pSF), &ItemID)) {
		hr = teGetDisplayNameBSTR(pSF, ItemID, uFlags & (~SHGDN_FORPARSINGEX), pbs);
		if (hr == S_OK) {
			if (teIsFileSystem(*pbs)) {
				if (uFlags & SHGDN_FORPARSINGEX) {
					BOOL bSearch = FALSE;
					BSTR bs2;
					if SUCCEEDED(teGetDisplayNameBSTR(pSF, ItemID, SHGDN_FORADDRESSBAR | SHGDN_FORPARSING, &bs2)) {
						bSearch = tePathMatchSpec1(bs2, L"search-ms:*");
						::SysFreeString(bs2);
					}
					if (!bSearch) {
				...
				if (ILGetCount(pidl) == 1 || tePathMatchSpec1(*pbs, L"search-ms:*\\*")) {
					LPITEMIDLIST pidl2 = teILCreateFromPath2(pSF, *pbs);
					if (!ILIsEqual(pidl, pidl2)) {
						teSysFreeString(pbs);
						hr = teGetDisplayNameBSTR(pSF, ItemID, uFlags & (~(SHGDN_FORPARSINGEX | SHGDN_FORADDRESSBAR)), pbs);
					}
					teCoTaskMemFree(pidl2);
				}
			}
...
BOOL teILIsExists(LPITEMIDLIST pidl)
{
	...
	if (GetShellFolder(&pSF, pidl)) {
		IEnumIDList *peidl;
		if SUCCEEDED(pSF->EnumObjects(g_nLockUpdate ? 0: g_hwndMain, SHCONTF_NONFOLDERS | SHCONTF_INCLUDEHIDDEN | SHCONTF_FOLDERS, &peidl)) {
			bResult = TRUE;
			peidl->Release();
		}
		pSF->Release();
...
/*///
			if (tePathIsNetworkPath(pszPath) && PathIsRoot(pszPath) && !PathIsDirectory(pszPath)) {
				teILFreeClear(&pidl);
			}
*///
/*/// To parse too much.
			...
			if (pidl == NULL && !teIsFileSystem(pszPath)) {
				...
				if (pidl == NULL) {
					...
					if (SHGetDesktopFolder(&pSF) == S_OK) {
						pidl = teILCreateFromPath2(pSF, pszPath);
						pSF->Release();
					}
					if (pidl == NULL) {
...
VOID CALLBACK teTimerFolderSize(HWND hwnd, UINT uMsg, UINT_PTR idEvent, DWORD dwTime)
{
	KillTimer(hwnd, idEvent);
	TEFS *pFS = (TEFS *)idEvent;
	if (pFS->pSB->m_pidl == pFS->pidl && pFS->pSB->m_pDispatch[SB_TotalFileSize]) {
		VARIANT v;
		teSetLL(&v, pFS->Result);
		tePutProperty(pFS->pSB->m_pDispatch[SB_TotalFileSize], pFS->bsName, &v);
		VariantClear(&v);
		InvalidateRect(pFS->pSB->m_hwndLV, NULL, FALSE);
	}
...
{
	TEFS *pFS = (TEFS *)args;
	pFS->Result = teGetFolderSize(pFS->bsPath, 128, pFS->pSB, pFS->pidl);
	SetTimer(g_hwndMain, (UINT_PTR)args, 100, teTimerFolderSize);
	::_endthread();
}
...
BOOL teGetIDListFromVariant(LPITEMIDLIST *ppidl, VARIANT *pv)
{
	*ppidl = NULL;
	switch(pv->vt) {
		case VT_UNKNOWN:
		...
		case VT_VARIANT | VT_BYREF:
			return teGetIDListFromVariant(ppidl, pv->pvarVal);
	}
	if (*ppidl == NULL) {
		VARIANT v;
		VariantInit(&v);
	...
	while (--i >= 0) {
		if (g_pSB[i]) {
			for (int j = Count_SBFunc; j-- > 1;) {
				if (g_pSB[i]->m_pDispatch[j]) {
					g_pSB[i]->m_pDispatch[j]->Release();
					g_pSB[i]->m_pDispatch[j] = NULL;
				}
			}
		}
		else {
...
BOOL GetVarPathFromFolderItem(FolderItem *pFolderItem, VARIANT *pVarResult)
{
	...
	if (pFolderItem) {
		...
		if SUCCEEDED(pFolderItem->QueryInterface(IID_PPV_ARGS(&v.pdispVal))) {
			...
			VariantClear(&v);
			if (pVarResult->vt == VT_BSTR && tePathMatchSpec1(pVarResult->bstrVal, L"search-ms:*")) {
				LPITEMIDLIST pidl;
...
VOID teApiOleCmdExec(int nArg, LONGLONG *param, DISPPARAMS *pDispParams, VARIANT *pVarResult)
{
	...
	if (FindUnknown(&pDispParams->rgvarg[nArg], &punk)) {
		...
		if SUCCEEDED(punk->QueryInterface(IID_PPV_ARGS(&pCT))) {
			GUID *pguid = NULL;
			CLSIDFromString((LPCOLESTR)param[1], pguid);
...
VOID teApiAdd(int nArg, LONGLONG *param, DISPPARAMS *pDispParams, VARIANT *pVarResult)
{
	teSetLL(pVarResult, param[0] + param[1]);
}

VOID teApiSub(int nArg, LONGLONG *param, DISPPARAMS *pDispParams, VARIANT *pVarResult)
{
	teSetLL(pVarResult, param[0] - param[1]);
}
...
VOID teApiGetProcObject(int nArg, LONGLONG *param, DISPPARAMS *pDispParams, VARIANT *pVarResult)
{
	...
	LPFNGetProcObjectW lpfnGetProcObjectW = (LPFNGetProcObjectW)GetProcAddress((HMODULE)param[0], lpProcNameA);
...
VOID teApiPSGetDisplayName(int nArg, LONGLONG *param, DISPPARAMS *pDispParams, VARIANT *pVarResult)
{
	if (param[0] && pVarResult) {
		PROPERTYKEY propKey;
		if SUCCEEDED(lpfnPSPropertyKeyFromStringEx((LPCWSTR)param[0], &propKey)) {
			pVarResult->bstrVal = tePSGetNameFromPropertyKeyEx(propKey, (int)param[1]);
			pVarResult->vt = VT_BSTR;
		}
	}
}
...
VOID teApiDoDragDrop(int nArg, LONGLONG *param, DISPPARAMS *pDispParams, VARIANT *pVarResult)
{
	IDataObject *pDataObj;
	if (GetDataObjFromVariant(&pDataObj, &pDispParams->rgvarg[nArg])) {
		g_pTE->m_bDrop = true;
		...
		try {
			teSetLong(pVarResult, ::DoDragDrop(pDataObj, static_cast<IDropSource *>(g_pTE), (DWORD)param[1], &dwEffect));
		}
		catch(...) {
		}
		if (pEffect) {
...
{
	teSetBSTR(pVarResult, SysAllocStringByteLen(reinterpret_cast<char*>(GetLPWSTRFromVariant(&pDispParams->rgvarg[nArg])), (UINT)param[1]), -2);
}
...
static void threadExists(void *args)
{
	...
	SetEvent(pExists->hEvent[tePathIsDirectory2(pExists->pszPath) ? 1 : 0]);
...
TEDispatchApi dispAPI[] = {
	...
	{ 2, -1, -1, -1, L"Add", teApiAdd },
	{ 2, -1, -1, -1, L"Sub", teApiSub },
	...
	{ 2, -1, -1, -1, L"GetProcObject", teApiGetProcObject },
	...
	{ 2, -1, -1, -1, L"SHParseDisplayName", teApiSHParseDisplayName  },
	...
	switch (idEvent) {
		case TET_Create:
			if (g_pOnFunc[TE_OnCreate]) {
				SetTimer(g_hwndMain, TET_ShowInit, 60 * 1000, teTimerProc);
				DoFunc(TE_OnCreate, g_pTE, E_NOTIMPL);
				DoFunc(TE_OnCreate, g_pWebBrowser, E_NOTIMPL);
			}
			else {
				WCHAR szError[1024];
				lstrcpy(szError, PathFileExists(g_pWebBrowser->m_bstrPath) ? L"500 Internal Script Error\n\n" : L"404 File Not Found\n\n");
				lstrcat(szError, g_pWebBrowser->m_bstrPath);
				MessageBox(g_hwndMain, szError, g_szTE, MB_OK);
...
VOID CALLBACK teTimerProcNavigate(HWND hwnd, UINT uMsg, UINT_PTR idEvent, DWORD dwTime)
{
	...
	CteShellBrowser *pSB = SBfromhwnd(hwnd);
	if (pSB) {
		pSB->BrowseObject(NULL, (UINT)idEvent);
	}
}
					...
					int       nCmdShow)
{
	...
	//Command Line
	BOOL bVisible = !tePathMatchSpec1(lpCmdLine, L"/run *");
	BOOL bNewProcess = tePathMatchSpec1(lpCmdLine, L"/open *");
...
";
	...
	if (bVisible) {
		...
		if (teIsFileSystem(lpFile) || tePathMatchSpec1(lpFile, L"file://*")) {
...
VOID CALLBACK teTimerProcParse(HWND hwnd, UINT uMsg, UINT_PTR idEvent, DWORD dwTime)
{
	...
	try {
		if (pidl || pInvoke->nErrorHandling == 3) {
			VariantClear(&pInvoke->pv[pInvoke->cArgs - 1]);
			FolderItem *pid;
			if (pidl && GetFolderItemFromPidl(&pid, pidl)) {
				teSetObjectRelease(&pInvoke->pv[pInvoke->cArgs - 1], pid);
			}
			teCoTaskMemFree(pidl);
			Invoke5(pInvoke->pdisp, pInvoke->dispid, DISPATCH_METHOD, NULL, pInvoke->cArgs, pInvoke->pv);
		}
		else if (pInvoke->nErrorHandling == 1) {
		...
		else if (pInvoke->nErrorHandling == 2) {
			if (g_bShowParseError) {
				...
				if (LoadString(g_hShell32, 6456, pszFormat, MAX_STATUS)) {
					if (FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_STRING | FORMAT_MESSAGE_ARGUMENT_ARRAY,
						...
						int r = MessageBox(g_hwndMain, lpBuf, APP_TITLE, MB_ABORTRETRYIGNORE);
						LocalFree(lpBuf);
						if (r == IDRETRY) {
					...
					if (pSB) {
						if (!pSB->m_bEmpty && pSB->m_nUnload == 0 && pSB->m_pidl) {
							...
							if SUCCEEDED(GetDisplayNameFromPidl(&v.bstrVal, pSB->m_pidl, SHGDN_FORADDRESSBAR | SHGDN_FORPARSING)) {
								if (tePathIsNetworkPath(v.bstrVal)) {
									pSB->Suspend(0);
								}
								::VariantClear(&v);
							}
...
/*	else if (IsEqualIID(riid, IID_IFolderFilter)) {
		*ppvObject = static_cast<IFolderFilter *>(this);
	}//*/
		...
		*ppvObject = static_cast<IServiceProvider *>(new CteServiceProvider(reinterpret_cast<IUnknown *>(this), m_pShellView));
		return S_OK;
	}
...
//		FALSE &&	
	...
	if (hr != S_OK) {
		...
		m_nLogIndex = m_nPrevLogIndex;
		if (m_pSF2 && m_pTabs && GetTabIndex() == m_pTabs->m_nIndex) {
			Show(TRUE, FALSE);
		}
		return hr;
	}
...
HRESULT CteShellBrowser::GetAbsPidl(LPITEMIDLIST *ppidlOut, FolderItem **ppid, FolderItem *pid, UINT wFlags, FolderItems *pFolderItems, FolderItem *pPrevious, CteShellBrowser *pHistSB)
{
	...
	if (wFlags & SBSP_NAVIGATEBACK) {
		...
		if (nLogIndex > 0 && teGetIDListFromObjectEx(pHistSB->m_ppLog[--nLogIndex], ppidlOut)) {
...
//			CheckNavigate(ppidlOut, pHistSB, nLogIndex);
	...
	if (wFlags & SBSP_NAVIGATEFORWARD) {
		int nLogIndex = pHistSB->m_nLogIndex;
		if (nLogIndex < pHistSB->m_nLogCount - 1 && teGetIDListFromObjectEx(pHistSB->m_ppLog[++nLogIndex], ppidlOut)) {
			pHistSB->m_ppLog[nLogIndex]->QueryInterface(IID_PPV_ARGS(ppid));
//			CheckNavigate(ppidlOut, pHistSB, nLogIndex);
			if (this == pHistSB) {
				m_nLogIndex = nLogIndex;
...
VOID CteShellBrowser::Refresh(BOOL bCheck)
{
	...
	if (m_pShellView) {
		if (m_bVisible) {
			...
			IFolderView2 *pFV2;
			if (m_dwUnavailable || (ILIsEqual(m_pidl, g_pidlResultsFolder) || (g_pidlLibrary && ILIsParent(g_pidlLibrary, m_pidl, FALSE)))) {
...
VOID CteShellBrowser::InitFolderSize()
{
	...
	if (m_hwndLV) {
		...
		if (hHeader) {
			...
			BSTR bsTotalFileSize = tePSGetNameFromPropertyKeyEx(PKEY_TotalFileSize, 0);
			BSTR bsLabel = g_pOnFunc[TE_Labels] ? tePSGetNameFromPropertyKeyEx(PKEY_Contact_Label, 0) : NULL;
			BSTR bsSize = tePSGetNameFromPropertyKeyEx(PKEY_Size, 0);
			HD_ITEM hdi;
			...
			for (int i = nHeader; i-- > 0;) {
				...
				if (g_bUpperVista) {
					...
					if (bsLabel && lstrcmpi(szText, bsLabel) == 0) {
						m_nLabelIndex = i;
					}
#ifdef _2000XP
				}
#endif
				if (bsSize && lstrcmpi(szText, bsSize) == 0) {
					m_nSizeIndex = i;
				}
			}
...
VOID CteShellBrowser::GetViewModeAndIconSize(BOOL bGetIconSize)
{
	...
	if (bGetIconSize || uViewMode != m_param[SB_ViewMode]) {
		...
		if (m_bCheckLayout && m_pExplorerBrowser) {
			IFolderViewOptions *pOptions;
			if SUCCEEDED(m_pExplorerBrowser->QueryInterface(IID_PPV_ARGS(&pOptions))) {
...
STDMETHODIMP CteShellBrowser::IncludeObject(IShellView *ppshv, LPCITEMIDLIST pidl)
{
		...
		if (m_pSF2) {
			...
			if SUCCEEDED(teGetDisplayNameBSTR(m_pSF2, pidl, SHGDN_INFOLDER | SHGDN_FORPARSING, &bs)) {
				if (!tePathMatchSpec(bs, m_bsFilter)) {
					hr = S_FALSE;
					BSTR bs2;
					if SUCCEEDED(teGetDisplayNameBSTR(m_pSF2, pidl, SHGDN_NORMAL, &bs2)) {
	...
	if (m_pDispatch[SB_OnIncludeObject]) {
		...
		if (teGetIDListFromObject(m_pSF2, &pidlParent)) {
		...
		Invoke4(m_pDispatch[SB_OnIncludeObject], &vResult, 2, pv);
		return GetIntFromVariantClear(&vResult);
	}
	return S_OK;
}
...
{
	*pdwFlags = (m_param[SB_ViewFlags] & (~CDB2GVF_NOINCLUDEITEM)) | ((m_bsFilter || m_pDispatch[SB_OnIncludeObject] || ILIsEqual(m_pidl, g_pidlResultsFolder)) ? 0 : CDB2GVF_NOINCLUDEITEM);
	return S_OK;
}
/*/// ICommDlgBrowser3
...
VOID CteShellBrowser::SetColumnsStr(BSTR bsColumns)
{
	...
	for (int i = nCount; i-- > 0;) {
		...
		if SUCCEEDED(lpfnPSPropertyKeyFromStringEx(lplpszArgs[i * 2], &propKey)) {
			methodArgs[i].name = tePSGetNameFromPropertyKeyEx(propKey, 0);
			pbAlloc[i] = TRUE;
		}
				...
				if SUCCEEDED(pColumnManager->GetColumns(CM_ENUM_VISIBLE, pPropKey, uCount)) {
					...
					for (UINT i = 0; i < uCount; i++) {
						if SUCCEEDED(pColumnManager->GetColumnInfo(pPropKey[i], &cmci)) {
							if (nFormat) {
								AddColumnDataEx(szColumns, tePSGetNameFromPropertyKeyEx(pPropKey[i], nFormat), cmci.uWidth);
							}
							else {
					...
					if (teGetIDListFromVariant(&pidl, &pDispParams->rgvarg[nArg])) {
						if (::ILIsParent(m_pidl, pidl, false)) {
							...
							if (n == 1) {
								...
								if (wfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
									if SUCCEEDED(teDelProperty(m_pDispatch[SB_TotalFileSize], wfd.cFileName)) {
										InvalidateRect(m_hwndLV, NULL, FALSE);
									}
								}
							}
...
/*			case DISPID_CONTENTSCHANGED:
			...
			default:
				if (dispIdMember >= START_OnFunc && dispIdMember < START_OnFunc + Count_SBFunc) {
					...
					if (nArg >= 0) {
						if (m_pDispatch[dispIdMember - START_OnFunc]) {
							m_pDispatch[dispIdMember - START_OnFunc]->Release();
							m_pDispatch[dispIdMember - START_OnFunc] = NULL;
						}
						IUnknown *punk;
						if (FindUnknown(&pDispParams->rgvarg[nArg], &punk)) {
...
BSTR CteShellBrowser::PSGetNameXP(LPWSTR pszName, int nFormat)
{
	...
	if (i >= 0) {
		...
		if SUCCEEDED(MapColumnToSCID(i, &scid)) {
			return tePSGetNameFromPropertyKeyEx(scid, nFormat);
		}
	}
	return NULL;
}
		...
		if (!m_dwUnavailable) {
			...
			try {
				...
				//		FALSE &&	
						...
						//ShellBrowser
						hr = NavigateSB(pPreviousView, m_pFolderItem);
						if SUCCEEDED(hr) {
							if (pPreviousView) {
								pPreviousView->DestroyViewWindow();
								pPreviousView->Release();
...
VOID CteShellBrowser::SetPropEx()
{
	if (m_pShellView->GetWindow(&m_hwndDV) == S_OK) {
		...
		if (m_hwndLV) {
			...
			hwndDT = FindWindowEx(m_hwndDV, NULL, L"DirectUIHWND", NULL);
		}
		if (!m_pDropTarget) {
			teRegisterDragDrop(hwndDT, this, &m_pDropTarget);
		}
	}
}
	...
	*pbs = NULL;
	...
	if SUCCEEDED(m_pShellView->QueryInterface(IID_PPV_ARGS(&pFV2))) {
		...
		if SUCCEEDED(pFV2->GetSortColumns(&col, 1)) {
			*pbs = tePSGetNameFromPropertyKeyEx(col.propkey, nFormat);
			if (col.direction < 0) {
				ToMinus(pbs);
			}
		}
	...
	if SUCCEEDED(MapColumnToSCID((UINT)Sort, &scid)) {
		*pbs = tePSGetNameFromPropertyKeyEx(scid, nFormat);
		HWND hHeader = ListView_GetHeader(m_hwndLV);
		if (hHeader) {
...
VOID CteShellBrowser::SetSort(BSTR bs)
{
	...
	if SUCCEEDED(lpfnPSPropertyKeyFromStringEx(szNew, &propKey)) {
		bsName = tePSGetNameFromPropertyKeyEx(propKey, 0);
		szNew = bsName;
	}
	BSTR bs1;
	GetSort(&bs1, 0);
						...
						case CTRL_TV:
							...
							while (--i >= 0) {
								if (g_pSB[i]) {
									if (!g_pSB[i]->m_bEmpty) {
										if (nCtrl == CTRL_TV) {
											teArrayPush(pArray, g_pSB[i]->m_pTV);
										}
										else {
											teArrayPush(pArray, g_pSB[i]);
										}
									}
								}
								else {
...
{
	return tePathMatchSpec1(pszUrl, L"http*") ? S_FALSE : S_OK;
}
...
STDMETHODIMP CteWebBrowser::QueryInterface(REFIID riid, void **ppvObject)
{
	*ppvObject = NULL;
	...
	else if (g_hwndMain == m_hwndParent && IsEqualIID(riid, IID_IDropTarget)) {
		*ppvObject = static_cast<IDropTarget *>(this);
	}
...
CteMemory::CteMemory(int nSize, void *pc, int nCount, LPWSTR lpStruct)
{
	...
	if (nSize > 0) {
		...
		if (pc == NULL) {
			...
			m_pc = reinterpret_cast<char *>(m_bsAlloc);
			if (m_pc) {
