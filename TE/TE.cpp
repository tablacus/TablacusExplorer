		...
		if (wfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
			...
			if (v.vt == VT_BSTR) {
				...
				if SUCCEEDED(m_pFolderItem->QueryInterface(g_ClsIdFI, (LPVOID *)&pid)) {
					pFS->pdwSessionId = &pid->m_dwSessionId;
					pFS->dwSessionId = pid->m_dwSessionId;
					pid->Release();
				}
				pFS->hwnd = m_hwndLV;
...
STDMETHODIMP CteShellBrowser::Invoke(DISPID dispIdMember, REFIID riid, LCID lcid, WORD wFlags, DISPPARAMS *pDispParams, VARIANT *pVarResult, EXCEPINFO *pExcepInfo, UINT *puArgErr)
{
	try {
		...
		case TE_METHOD + 0xf501://AddItem
			if (nArg >= 0) {
				if (nArg >= 1) {
					...
					CteFolderItem *pid;
					if SUCCEEDED(m_pFolderItem->QueryInterface(g_ClsIdFI, (LPVOID *)&pid)) {
						dwSessionId = pid->m_dwSessionId;
						pid->Release();
					}
					if (dwSessionId != GetIntFromVariant(&pDispParams->rgvarg[0])) {
						return E_ACCESSDENIED;
					}
				}
				LPITEMIDLIST pidl;
		...
		case TE_METHOD + 0xf503://AddItems
			if (nArg >= 0) {
				if (GetDispatch(&pDispParams->rgvarg[nArg], &pdisp) || teGetDispatchFromString(&pDispParams->rgvarg[nArg], &pdisp)) {
					...
					CteFolderItem *pid;
					if SUCCEEDED(m_pFolderItem->QueryInterface(g_ClsIdFI, (LPVOID *)&pid)) {
						teSetLong(&pAddItems->pv[0], pid->m_dwSessionId);
						pid->Release();
					}
		...
		case TE_PROPERTY + 0xf505://SessionId
			CteFolderItem *pid1;
			teQueryFolderItem(&m_pFolderItem, &pid1);
			teSetLong(pVarResult, pid1->m_dwSessionId);
			pid1->Release();
			return S_OK;
			//
		case DISPID_VALUE:
...
HRESULT CteShellBrowser::RemoveAll()
{
	...
	if (m_pShellView) {
		...
		CteFolderItem *pid;
		if SUCCEEDED(m_pFolderItem->QueryInterface(g_ClsIdFI, (LPVOID *)&pid)) {
			pid->m_dwSessionId = MAKELONG(GetTickCount(), rand());
			pid->Release();
		}
