...
HRESULT CteShellBrowser::Navigate2(FolderItem *pFolderItem, UINT wFlags, DWORD *param, FolderItems *pFolderItems, FolderItem *pPrevious, CteShellBrowser *pHistSB)
{
	...
	g_dwTickFocus = 0;
	m_dwSorting = ILIsEqual(pidl, g_pidls[CSIDL_RESULTSFOLDER]) ? TESORTING_RESULTS : 0;
#ifdef _2000XP
	if (g_bUpperVista) {
#endif
...
VOID CteShellBrowser::OnNavigationComplete2()
{
	...
	SetFolderFlags(FALSE);
	InitFolderSize();
	if (!(m_dwSorting & (TESORTING_SORT | TESORTING_GROUP))
#ifdef _2000XP
		|| !g_bUpperVista
#endif
	) {
...
VOID CteShellBrowser::Suspend(int nMode)
{
	...
	m_bsFilter = NULL;
	m_pTC->LockUpdate(TEREDRAW_SUSPEND);
	try {
		DestroyView(0);
		if (nMode == 4) {
			m_nUnload = 8;
...
VOID CteShellBrowser::SetGroupBy(BSTR bs)
{
	...
	if SUCCEEDED(m_pShellView->QueryInterface(IID_PPV_ARGS(&pFV2))) {
		if SUCCEEDED(PropertyKeyFromName(szNew, &propKey)) {
			if (m_dwSorting && !IsEqualPropertyKey(propKey, PKEY_Null)) {
				teSysFreeString(&m_bsNextGroup);
				m_bsNextGroup = ::SysAllocString(bs);
				SetTimer(m_hwnd, (UINT_PTR)&m_bsNextGroup, 10000, teTimerProcGroupBy);
			} else {
