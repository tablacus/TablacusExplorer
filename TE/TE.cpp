...
LPITEMIDLIST teILCreateFromPath3(IShellFolder *pSF, LPWSTR pszPath, HWND hwnd)
{
	...
	int ashgdn[] = { SHGDN_INFOLDER | SHGDN_FORPARSING, SHGDN_FORADDRESSBAR | SHGDN_FORPARSING | SHGDN_INFOLDER };
				...
				if (lpDispInfo->item.mask & LVIF_TEXT) {
					...
					if (lpDispInfo->item.iSubItem == 0 && (pSB->m_dwNameFormat || !g_sfs.fShowExtensions)) {
						...
						if (pSB->m_dwNameFormat & 1) {
							...
							try {
								while (lpEsc) {
									swscanf_s(&lpEsc[1], L"%02hx", lpEsc);
									lstrcpy(&lpEsc[1], &lpEsc[3]);
									lpEsc = StrChr(&lpEsc[1], '%');
								}
							} catch (...) {}
						}
						return lResult;
	...
	if (param[0].lpcwstr) {
		...
		if SUCCEEDED(lpfnPSPropertyKeyFromStringEx(param[0].lpcwstr, &propKey)) {
			...
			if (lpfnPSGetPropertyDescription) {
				...
				if SUCCEEDED(lpfnPSGetPropertyDescription(propKey, IID_PPV_ARGS(&pdesc))) {
					LPWSTR psz;
					if SUCCEEDED(pdesc->FormatForDisplay(*((PROPVARIANT *)&pDispParams->rgvarg[nArg - 1]), param[2].propdesc_format_flags, &psz)) {
				...
				IPropertyUI *pPUI;
				if SUCCEEDED(CoCreateInstance(CLSID_PropertiesUI, NULL, CLSCTX_INPROC, IID_PPV_ARGS(&pPUI))) {
					pPUI->FormatForDisplay(propKey.fmtid, propKey.pid, (PROPVARIANT *)&pDispParams->rgvarg[nArg - 1], param[2].propertyui_format_flags, bsResult, MAX_PROP);
					pPUI->Release();
				}
				teSetBSTR(pVarResult, &bsResult, -1);
			}
...
{
	int nLen = param[1].intVal;
	if (nLen <= 0) {
		nLen = MAX_COLUMN_NAME_LEN;
	}
	BSTR bsResult = ::SysAllocStringLen(NULL, nLen);
	StrFormatByteSize(param[0].llVal, bsResult, nLen);
	teSetBSTR(pVarResult, &bsResult, -1);
}
...
VOID teApiSHSimpleIDListFromPath(int nArg, teParam *param, DISPPARAMS *pDispParams, VARIANT *pVarResult)
{
	...
	if (pidl) {
		int nSize = ILGetSize(pidl);
		if (nArg >= 0 && nSize > 16) {
			LPWORD pwIdl = (LPWORD)ILFindLastID(pidl);
			pwIdl[6] = param[1].word;
			if (nArg >= 2) {
				DATE dt;
				...
				if (nArg >= 3) {
					*((UINT *)&pwIdl[2]) = param[3].uintVal;
#ifdef _2000XP
					if (g_bUpperVista) {
#endif
						if (nSize > 0x39 && param[3].uli.HighPart) {
							*((UINT *)&pwIdl[27]) =  param[3].uli.HighPart;
						}
#ifdef _2000XP
					}
#endif
...
ATOM MyRegisterClass(HINSTANCE hInstance, LPWSTR szClassName, WNDPROC lpfnWndProc)
{
	...
	wcex.hIconSm		= CopyIcon(wcex.hIcon);
		...
		if (pImageCodecInfo) {
			...
			while (num--)
			{
				if (tePathMatchSpec(pszName, pImageCodecInfo[num].FilenameExtension) || lstrcmpi(pszName, pImageCodecInfo[num].MimeType) == 0) {
					...
					if (pszMimeType) {
						lstrcpyn(pszMimeType, pImageCodecInfo[num].MimeType, 31);
					}
	...
	try {
		if (pInvoke->pidl || pInvoke->wErrorHandling == 3) {
			...
			if (pInvoke->wMode) {
				...
				FolderItem *pid;
				if (pInvoke->pidl && GetFolderItemFromIDList(&pid, pInvoke->pidl)) {
					teSetObjectRelease(&pInvoke->pv[pInvoke->cArgs - 1], pid);
				}
			}
...
HRESULT CteShellBrowser::Navigate2(FolderItem *pFolderItem, UINT wFlags, DWORD *param, FolderItems *pFolderItems, FolderItem *pPrevious, CteShellBrowser *pHistSB)
{
	...
	SafeRelease(&m_pDispatch[SB_AltSelectedItems]);
	CteFolderItem *pid1 = NULL;
	if (m_hwnd) {
	...
	if (param[SB_DoFunc]) {
		...
		if (!teILIsEqual(m_pFolderItem, pPrevious)) {
			teSysFreeString(&m_bsFilter);
			SafeRelease(&m_pDispatch[SB_OnIncludeObject]);
		}
	}
					...
					if (teGetIDListFromVariant(&pidl, &pDispParams->rgvarg[nArg])) {
						if (!ILIsEmpty(pidl)) {
							...
							if (pidlChild) {
								...
								FolderItem *pFolderItem;
								if (GetFolderItemFromIDList(&pFolderItem, pidlFull)) {
									teSetObjectRelease(pVarResult, pFolderItem);
								}
								teCoTaskMemFree(pidlFull);
...
/*			case DISPID_CONTENTSCHANGED://XP-
			...
			default:
				if (dispIdMember >= START_OnFunc && dispIdMember < START_OnFunc + Count_SBFunc) {
					...
					if (nArg >= 0) {
						SafeRelease(&m_pDispatch[dispIdMember - START_OnFunc]);
						IUnknown *punk;
						if (FindUnknown(&pDispParams->rgvarg[nArg], &punk)) {
...
*///
			...
			default:
				if (dispIdMember >= START_OnFunc && dispIdMember < START_OnFunc + Count_OnFunc) {
					...
					if (nArg >= 0) {
						SafeRelease(&g_pOnFunc[dispIdMember - START_OnFunc]);
						if (FindUnknown(&pDispParams->rgvarg[nArg], &punk)) {
...
VOID CteGdiplusBitmap::ClearImage()
{
#ifdef _USE_WIC
...
BOOL CteGdiplusBitmap::GetBGRA()
{
	WICPixelFormatGUID guidPF;
	m_pImage->GetPixelFormat(&guidPF);
	if (IsEqualGUID(guidPF, GUID_WICPixelFormat32bppBGRA)) {
		return TRUE;
	}
	BOOL b = FALSE;
	IWICFormatConverter *pConverter;
	if SUCCEEDED(g_pWICFactory->CreateFormatConverter(&pConverter)) {
		if SUCCEEDED(pConverter->Initialize(m_pImage, GUID_WICPixelFormat32bppBGRA, WICBitmapDitherTypeNone, NULL, 0.f, WICBitmapPaletteTypeMedianCut)) {
...
HRESULT CteGdiplusBitmap::CreateStream(IStream *pStream, ULARGE_INTEGER *puliSize, CLSID encoderClsid, LONG lQuality)
{
	HRESULT hr = E_FAIL;
	if (pStream) {
		...
		if SUCCEEDED(g_pWICFactory->CreateEncoder(encoderClsid, NULL, &pEncoder)) {
			...
			if (m_pDecoder) {
				CLSID guidSrc;
				m_pDecoder->GetContainerFormat(&guidSrc);
				if (IsEqualGUID(encoderClsid, guidSrc)) {
					UINT uCount;
					if (SUCCEEDED(m_pDecoder->GetFrameCount(&uCount)) && uCount > 1) {
						if (IsEqualGUID(encoderClsid, GUID_ContainerFormatGif)) {
							IWICMetadataQueryWriter *pMetadataQueryWriter;
							if SUCCEEDED(pEncoder->GetMetadataQueryWriter(&pMetadataQueryWriter)) {
								IWICMetadataQueryReader *pMetadataQueryReader;
								if SUCCEEDED(m_pDecoder->GetMetadataQueryReader(&pMetadataQueryReader)) {
									PROPVARIANT v;
									PropVariantInit(&v);
									LPWSTR lpData[] = {L"/appext/Application", L"/appext/Data"};
									for (int i = 0; i < 2; i++) {
										if SUCCEEDED(pMetadataQueryReader->GetMetadataByName(lpData[i], &v)) {
											pMetadataQueryWriter->SetMetadataByName(lpData[i], &v);
											PropVariantClear(&v);
										}
									}
...
/*								v.vt = VT_UI1 | VT_VECTOR;
								v.caub.cElems = 11;
								v.caub.pElems = new UCHAR[11];
								::CopyMemory(v.caub.pElems, "NETSCAPE2.0", 11);
								pMetadataQueryWriter->SetMetadataByName(L"/appext/Application", &v);
//								delete [] v.caub.pElems;
								//v.vt = VT_UI1 | VT_VECTOR;
								v.caub.cElems = 5;
//								v.caub.pElems = new UCHAR[5];
								v.caub.pElems[0] = 3;
								v.caub.pElems[1] = 1;
								v.caub.pElems[2] = 0;
								v.caub.pElems[3] = 0;
								v.caub.pElems[4] = 0;
								pMetadataQueryWriter->SetMetadataByName(L"/appext/Data", &v);
								delete [] v.caub.pElems;*/
								SafeRelease(&pMetadataQueryWriter);
							}
						}
						for (UINT i = 0; i < uCount; i++) {
							IWICBitmapFrameDecode *pFrameDecode;
							hr = m_pDecoder->GetFrame(i, &pFrameDecode);
							if SUCCEEDED(hr) {
...
/*
HRESULT CteGdiplusBitmap::CreateBMPStream(IStream *pStream, ULARGE_INTEGER *puliSize, LPWSTR szMime)
{
	HRESULT hr = E_FAIL;
	if (pStream) {
...
*/
BOOL CteGdiplusBitmap::HasImage()
{
	if (m_pImage) {
		UINT w = 0, h = 0;
		m_pImage->GetSize(&w, &h);
...
BOOL CteGdiplusBitmap::HasImage()
{
	return m_pImage && m_pImage->GetWidth();
}
#endif

CteGdiplusBitmap* CteGdiplusBitmap::GetBitmapObj()
{
...
#ifdef _USE_WIC
	SafeRelease(&m_pDecoder);
	if SUCCEEDED(g_pWICFactory->CreateDecoderFromStream(pStream, NULL, WICDecodeMetadataCacheOnDemand, &m_pDecoder)) {
		IWICBitmapFrameDecode *pFrameDecode;
		if SUCCEEDED(m_pDecoder->GetFrame(0, &pFrameDecode)) {
			SafeRelease(&m_pImage);
...
#ifdef _USE_WIC
					SafeRelease(&m_pDecoder);
					if SUCCEEDED(g_pWICFactory->CreateDecoderFromFilename(GetLPWSTRFromVariant(&pDispParams->rgvarg[nArg]), NULL, GENERIC_READ, WICDecodeMetadataCacheOnDemand, &m_pDecoder)) {
						IWICBitmapFrameDecode *pFrameDecode;
						if SUCCEEDED(m_pDecoder->GetFrame(0, &pFrameDecode)) {
							SafeRelease(&m_pImage);
							g_pWICFactory->CreateBitmapFromSource(pFrameDecode, WICBitmapCacheOnDemand, &m_pImage);
			...
			case 100:
				if (nArg >= 0) {
					...
					if (GetEncoderClsid(vText.bstrVal, &encoderClsid, NULL)) {
						...
						IStream *pStream;
						hr = SHCreateStreamOnFileEx(vText.bstrVal, STGM_READWRITE | STGM_SHARE_DENY_WRITE, FILE_ATTRIBUTE_NORMAL, TRUE, NULL, &pStream);
						if SUCCEEDED(hr) {
						...
						hr = m_pImage->Save(vText.bstrVal, &encoderClsid, pEncoderParameters);
#endif
					}
				}
				teSetLong(pVarResult, hr);
...
#ifdef _USE_WIC
				if (m_pDecoder) {
					m_pDecoder->GetFrameCount(&pVarResult->uintVal);
				}
#endif
				return S_OK;
			//GetThumbnailImage
