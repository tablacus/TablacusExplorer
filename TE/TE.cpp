	...
	if (lpfnPSGetPropertyDescription) {
		...
		if SUCCEEDED(lpfnPSGetPropertyDescription(propKey, IID_PPV_ARGS(&pdesc))) {
			...
			if (nFormat == 0 && pSB && bs) {
				...
				if SUCCEEDED(pSB->PropertyKeyFromName(bs, &propKey1)) {
...
VOID teApiWideCharToMultiByte(int nArg, teParam *param, DISPPARAMS *pDispParams, VARIANT *pVarResult)
{
	if (pVarResult) {
		UCHAR *pc = NULL;
		GetpDataFromVariant(&pc, &pDispParams->rgvarg[nArg - 1]);
		if (pc) {
...
VOID teApiURLDownloadToFile(int nArg, teParam *param, DISPPARAMS *pDispParams, VARIANT *pVarResult)
{
	...
	if (FindUnknown(&pDispParams->rgvarg[nArg - 1], &punk)) {
		IStream *pDst, *pSrc;
		HRESULT hr = punk->QueryInterface(IID_PPV_ARGS(&pSrc));
		if SUCCEEDED(hr) {
			...
			case TET_Size:
				...
				try {
					for (UINT i = 0; i < g_ppTC.size(); ++i) {
						CteTabCtrl *pTC = g_ppTC[i];
						if (pTC->m_bVisible) {
							pTC->LockUpdate(TEREDRAW_NORMAL);
							try {
...
VOID CALLBACK teTimerProcSortOrGroup(HWND hwnd, UINT uMsg, UINT_PTR idEvent, DWORD dwTime)
{
	try {
		KillTimer(hwnd, idEvent);
		CteShellBrowser *pSB = SBfromhwnd(hwnd);
		if (pSB && pSB->m_bsNextValue && idEvent == (UINT_PTR)&pSB->m_bsNextValue) {
			pSB->m_dwSorting = 0;
			VARIANT *pv = GetNewVARIANT(1);
			teSetSZ(&pv[0], pSB->m_bsNextValue);
			teSysFreeString(&pSB->m_bsNextValue);
			Invoke5(pSB, pSB->m_dispidSortOrGroup, DISPATCH_PROPERTYPUT, NULL, 1, pv);
		}
	} catch (...) {
		g_nException = 0;
#ifdef _DEBUG
		g_strException = L"teTimerProcSortOrGroup";
#endif
	}
}
...
VOID CALLBACK teTimerProcForStatic(HWND hwnd, UINT uMsg, UINT_PTR idEvent, DWORD dwTime)
{
	KillTimer(hwnd, idEvent);
	try {
		CteTabCtrl *pTC = TCfromhwnd(hwnd);
		...
		g_nException = 0;
#ifdef _DEBUG
		g_strException = L"teTimerProcForStatic";
#endif
	}
}
...
STDMETHODIMP CteShellBrowser::Invoke(DISPID dispIdMember, REFIID riid, LCID lcid, WORD wFlags, DISPPARAMS *pDispParams, VARIANT *pVarResult, EXCEPINFO *pExcepInfo, UINT *puArgErr)
{
	try {
		...
		if ((m_nUnload == 1 || (m_nUnload & 8)) && dispIdMember >= 0x10000100 && dispIdMember) {
			return S_OK;
		}
...
//*///
			...
			case DISPID_CONTENTSCHANGED://XP-
				m_dwSorting &= ~TESORTING_GROUP;
				if (m_bsNextValue) {
					teTimerProcSortOrGroup(m_hwnd, 0, (UINT_PTR)&m_bsNextValue, 0);
				}
				if (m_pShellView) {
					IFolderView2 *pFV2;
					if SUCCEEDED(m_pShellView->QueryInterface(IID_PPV_ARGS(&pFV2))) {
			...
			case DISPID_SORTDONE://XP-
				m_dwSorting &= ~TESORTING_SORT;
				if (m_hwndLV) {
					if (m_param[SB_ViewFlags] & CDB2GVF_NOSELECTVERB) {
						ListView_SetSelectedColumn(m_hwndLV, -1);
					}
				}
				if (m_bsNextValue && m_dispidSortOrGroup == 0x10000105/*GroupBy*/) {
					SetTimer(m_hwnd, (UINT_PTR)&m_bsNextValue, 250, teTimerProcSortOrGroup);
				}
				m_pTC->RedrawUpdate();
				if (m_pShellView) {
					IFolderView2 *pFV2;
		...
		if (m_pdisp) {
			if (dispIdMember == m_dispidSortColumns && (wFlags & DISPATCH_PROPERTYPUT) && nArg >= 0 && pDispParams->rgvarg[nArg].vt == VT_BSTR) {
				...
				VariantClear(&v);
				if (iChanged) {
					if (m_dwSorting & TESORTING_GROUP) {
						teSysFreeString(&m_bsNextValue);
						m_bsNextValue = ::SysAllocString(pDispParams->rgvarg[nArg].bstrVal);
						m_dispidSortOrGroup = dispIdMember;
						SetTimer(m_hwnd, (UINT_PTR)&m_bsNextValue, 10000, teTimerProcSortOrGroup);
					} else {
						m_dwSorting |= TESORTING_SORT;
					}
				} else {
...
HRESULT CteShellBrowser::Items(UINT uItem, FolderItems **ppid)
{
	...
	if (m_pShellView) {
		...
		if (nCount || !m_hwndLV || uItem != SVGIO_SELECTION) {
#ifdef _2000XP
	...
	if SUCCEEDED(m_pShellView->QueryInterface(IID_PPV_ARGS(&pFV2))) {
		if SUCCEEDED(PropertyKeyFromName(szNew, &pCol1[0].propkey)) {
			...
			if (!teIsSameSort(pFV2, pCol, nSortColumns)) {
				if (m_dwSorting & TESORTING_GROUP) {
					teSysFreeString(&m_bsNextValue);
					m_bsNextValue = ::SysAllocString(bs);
					m_dispidSortOrGroup = 0x10000104;//SortColumn
					SetTimer(m_hwnd, (UINT_PTR)&m_bsNextValue, 10000, teTimerProcSortOrGroup);
				} else {
					m_dwSorting |= TESORTING_SORT;
					m_nFocusItem = -1;
...
VOID CteShellBrowser::SetGroupBy(BSTR bs)
{
	...
	if SUCCEEDED(m_pShellView->QueryInterface(IID_PPV_ARGS(&pFV2))) {
		if SUCCEEDED(PropertyKeyFromName(szNew, &propKey)) {
			if (m_dwSorting & TESORTING_SORT) {
				teSysFreeString(&m_bsNextValue);
				m_bsNextValue = ::SysAllocString(bs);
				m_dispidSortOrGroup = 0x10000105/*GroupBy*/;
				SetTimer(m_hwnd, (UINT_PTR)&m_bsNextValue, 10000, teTimerProcSortOrGroup);
			} else {
				m_dwSorting |= TESORTING_GROUP;
		...
		switch(dispIdMember) {
				...
				if (nArg >= 1) {
					...
					if (lpProc) {
						if (lstrcmpi(lpMethod, L"GetArchive") == 0) {
							if (dispIdMember == TE_METHOD + 1025) {
								g_ppGetArchive.push_back(lpProc);
							} else {
								for (size_t u = g_ppGetArchive.size(); u--;) {
									if (g_ppGetArchive[u] == lpProc) {
										g_ppGetArchive.erase(g_ppGetArchive.begin() + u);
									}
								}
							}
						} else if (lstrcmpi(lpMethod, L"GetImage") == 0) {
							if (dispIdMember == TE_METHOD + 1025) {
								g_ppGetImage.push_back(lpProc);
							} else {
								for (size_t u = g_ppGetImage.size(); u--;) {
									if (g_ppGetImage[u] == lpProc) {
										g_ppGetImage.erase(g_ppGetImage.begin() + u);
									}
								}
							}
						}
			...
			case TE_METHOD + 1136:
				if (nArg >= 0 && GetDispatch(&pDispParams->rgvarg[nArg], &pdisp)) {
					CteDispatch *oDisp = new CteDispatch(pdisp, 1);
					pdisp->Release();
					teSetObjectRelease(pVarResult, oDisp);
				}
				return S_OK;
...
/*			case 1150:
			...
			case 1160://DragIcon
				if (nArg >= 0) {
					g_bDragIcon = GetIntFromVariant(&pDispParams->rgvarg[nArg]);
#ifdef _2000XP
					if (g_bUpperVista) {
#endif
						if (g_bDragIcon) {
...
VOID CteTabCtrl::Show(BOOL bVisible)
{
	...
	if (bVisible ^ m_bVisible) {
		...
		m_bVisible = bVisible;
		SetTimer(m_hwndStatic, TET_VisibleChanged, 100, teTimerProcForStatic);
	}
}
...
STDMETHODIMP CteTabCtrl::Invoke(DISPID dispIdMember, REFIID riid, LCID lcid, WORD wFlags, DISPPARAMS *pDispParams, VARIANT *pVarResult, EXCEPINFO *pExcepInfo, UINT *puArgErr)
{
	try {
		...
		switch(dispIdMember) {
			...
			//LockUpdate
			case 13:
				LockUpdate(nArg >= 0 ? GetIntFromVariant(&pDispParams->rgvarg[nArg]) : TEREDRAW_NORMAL);
				return S_OK;
			//UnlockUpdate
			case 14:
				UnlockUpdate();
...
VOID CteTabCtrl::LockUpdate(int nRedraw)
{
	m_nRedraw |= nRedraw;
...
VOID CteTabCtrl::RedrawUpdate()
{
	if (m_nLockUpdate || g_nLockUpdate || (m_nRedraw & TEREDRAW_NAVIGATE) || !(m_nRedraw & TEREDRAW_NORMAL)) {
		SetTimer(m_hwndStatic, TET_Check, 10000, teTimerProcForStatic);
		return;
	}
	...
	KillTimer(m_hwndStatic, TET_Check);
