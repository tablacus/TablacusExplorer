...
{
	LPITEMIDLIST pidl;
	pidl = teSHSimpleIDListFromPath(param[0].lpwstr, nArg >= 1 && (param[1].dword & FILE_ATTRIBUTE_DIRECTORY), 0);
	if (pidl) {
		int nSize = ILGetSize(pidl);
		if (nArg >= 0 && nSize > 16) {
			LPWORD pwIdl = (LPWORD)ILFindLastID(pidl);
...
BOOL CteShellBrowser::Navigate1(FolderItem *pFolderItem, UINT wFlags, FolderItems *pFolderItems, FolderItem *pPrevious, LPITEMIDLIST *ppidl, int nErrorHandling)
{
	...
	if (pFolderItem) {
		if SUCCEEDED(pFolderItem->QueryInterface(g_ClsIdFI, (LPVOID *)&pid)) {
			if (!pid->m_pidl && !pid->m_pidlAlt) {
				if (pid->m_v.vt == VT_BSTR) {
					...
					if (ppidl) {
						pid->GetPidl();
						*ppidl = ILClone(pid->GetAlt());
						if (!*ppidl && m_pShellView) {
							teILFreeClear(ppidl);
							Navigate1Ex(pid->m_v.bstrVal, pFolderItems, wFlags, pPrevious, nErrorHandling);
							pid->Release();
							return TRUE;
		...
		if (ppidl) {
			teGetIDListFromObjectForFV(pFolderItem, ppidl);
		}
	}
	return FALSE;
}
...
HRESULT CteShellBrowser::GetAbsPidl(LPITEMIDLIST *ppidlOut, FolderItem *pid, UINT wFlags, FolderItems *pFolderItems, FolderItem *pPrevious, CteShellBrowser *pHistSB)
{
	...
	if (m_pFolderItem1) {
		return teGetIDListFromObjectForFV(m_pFolderItem1, ppidlOut) ? S_OK : E_FAIL;
	}
	return E_FAIL;
}
...
{
	teILCloneReplace(&m_pidlAlt, g_pidlResultsFolder);
	m_dwUnavailable = GetTickCount();
}
...
STDMETHODIMP CteFolderItem::Invoke(DISPID dispIdMember, REFIID riid, LCID lcid, WORD wFlags, DISPPARAMS *pDispParams, VARIANT *pVarResult, EXCEPINFO *pExcepInfo, UINT *puArgErr)
{
	...
	try {
		...
		switch(dispIdMember) {
			...
			case 6:
				if (nArg >= 0) {
					GetDispatch(&pDispParams->rgvarg[nArg], &m_pENum);
				}
				if (pVarResult) {
					GetAlt();
					teSetObject(pVarResult, m_pENum);
				}
				return S_OK;
