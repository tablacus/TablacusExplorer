	...
	if (pSF2) {
		...
		if (wfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
			...
			if (v.vt == VT_BSTR) {
				...
				CteFolderItem *pid1;
				teQueryFolderItem(&m_pFolderItem, &pid1);
				pFS->pdwSessionId = &pid1->m_dwSessionId;
				pFS->dwSessionId = pid1->m_dwSessionId;
				pid1->Release();
				pFS->hwnd = m_hwndLV;
		...
		case TE_METHOD + 0xf503://AddItems
			if (nArg >= 0) {
				if (GetDispatch(&pDispParams->rgvarg[nArg], &pdisp) || teGetDispatchFromString(&pDispParams->rgvarg[nArg], &pdisp)) {
					...
					CteFolderItem *pid1;
					teQueryFolderItem(&m_pFolderItem, &pid1);
					teSetLong(&pAddItems->pv[0], pid1->m_dwSessionId);
					pid1->Release();
...
HRESULT CteShellBrowser::RemoveAll()
{
	...
	if (m_pShellView) {
		...
		CteFolderItem *pid1;
		teQueryFolderItem(&m_pFolderItem, &pid1);
		pid1->m_dwSessionId = MAKELONG(GetTickCount(), rand());
		pid1->Release();
