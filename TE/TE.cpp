...
method methodTE[] = {
	...
	{ START_OnFunc + TE_OnCopyData, L"OnCopyData" },
...
BOOL GetDispatch(VARIANT *pv, IDispatch **ppdisp)
{
	...
	if (pv->vt == (VT_VARIANT | VT_BYREF)) {
		return GetDispatch(pv->pvarVal, ppdisp);
	}
	return false;
}
...
BOOL GetMemFormVariant(VARIANT *pv, CteMemory **ppMem)
{
	...
	if (GetDispatch(pv, &pdisp)) {
		return SUCCEEDED(pdisp->QueryInterface(g_ClsIdStruct, (LPVOID *)ppMem));
	}
	return FALSE;
}
...
LONGLONG GetLLFromVariant(VARIANT *pv)
{
	if (pv) {
		...
		if (pv->vt == (VT_ARRAY | VT_I4)) {
			LONGLONG ll = 0;
			::CopyMemory(&ll, pv->parray->pvData, sizeof(LONGLONG));
			return ll;
		}
		if (pv->vt == VT_I8) {
			return pv->llVal;
	...
	if (!g_bsCmdLine) {
		...
		while (i < nSize) {
			if (StrCmpNI(&strCmdLine[j], L"/idlist,:", 9) == 0) {
				...
				DWORD dwProcessID;
				WCHAR sz[MAX_PATH + 2];
				swscanf_s(&strCmdLine[j + 9], L"%32[1234567890]s", &sz);
				swscanf_s(sz, L"%lld", &hData);
				int k = lstrlen(sz);
				swscanf_s(&strCmdLine[j + k + 10], L"%32[1234567890]s", &sz);
				...
				if (pidl) {
					...
					lstrcpy(&g_bsCmdLine[i], v2.bstrVal);
					i += lstrlen(v2.bstrVal);
...
HWND FindTreeWindow(HWND hwnd)
{
	...
	return FindWindowEx(FindWindowEx(hwnd, 0, L"NamespaceTreeControl", NULL), 0, WC_TREEVIEW, NULL);
}
...
LPITEMIDLIST teILCreateFromPath2(IShellFolderPtr pSF, LPWSTR pszPath, SHGDNF uFlags)
{
...
//		SHParseDisplayName(pszPath, NULL, &pidl, 0, NULL);
...
{
	int nLen = SysStringLen(*pbs) + 1;
	LPWSTR szCol = new WCHAR[nLen + 1];
	szCol[0] = '-';
	::CopyMemory(&szCol[1], *pbs, nLen * sizeof(WCHAR));
		...
		if (pDataObj->GetData(&IDLISTFormat, &Medium) == S_OK) {
			...
			*pnCount = DragQueryFile((HDROP)Medium.hGlobal, (UINT)-1, NULL, 0);
			...
			for (int i = *pnCount - 1; i >= 0; i--) {
				...
				ppidllist[i + 1] = teILCreateFromPath(szPath);
			}
			ReleaseStgMedium(&Medium);
		}
	}
...
BOOL GetPidlFromVariant(LPITEMIDLIST *ppidl, VARIANT *pv)
{
	*ppidl = NULL;
	switch(pv->vt) {
		...
		case VT_DISPATCH:
			if (pv->pdispVal) {
				GetIDListFromObject(pv->pdispVal, ppidl);
			}
			break;
		case VT_ARRAY | VT_I1:
		case VT_ARRAY | VT_UI1:
			...
			SafeArrayGetLBound(pv->parray, 1, &lLBound);
			nSize = lUBound - lLBound + 1;
			*ppidl = (LPITEMIDLIST)CoTaskMemAlloc(nSize);
			::CopyMemory(*ppidl, pv->parray, nSize);
			break;
		case VT_NULL:
			break;
		case VT_LPWSTR:
	...
	if (*ppidl == NULL) {
		...
		if (SUCCEEDED(VariantChangeType(&v, pv, 0, VT_I4))) {
			if (v.lVal < MAX_CSIDL) {
				SHGetSpecialFolderLocation(NULL, v.lVal, ppidl);
			}
		}
	}
	return (*ppidl != NULL);
...
BOOL GetDataObjFromVariant(IDataObject **ppDataObj, VARIANT *pv)
{
	*ppDataObj = NULL;
	IDispatch *pdisp;
	if (GetDispatch(pv, &pdisp)) {
		if SUCCEEDED(pdisp->QueryInterface(IID_PPV_ARGS(ppDataObj))) {
			return true;
		}
		FolderItems *pItems;
...
BOOL GetVariantFromShellItem(VARIANT *pv, IShellItem *psi)
{
	...
	if SUCCEEDED(GetFolderItemFromShellItem(&pid, psi)) {
		if SUCCEEDED(pid->QueryInterface(IID_PPV_ARGS(&pv->punkVal))) {
			pv->vt = VT_UNKNOWN;
			return true;
		}
	}
...
VOID SetVariantLL(VARIANT *pv, LONGLONG ll)
{
	if (pv) {
		...
		if (psa) {
			::CopyMemory(psa->pvData, &ll, sizeof(LONGLONG));
...
HRESULT DoFunc(int nFunc, IUnknownPtr punk, HRESULT hr)
{
	if (g_pOnFunc[nFunc]) {
		VARIANT vResult;
		VariantInit(&vResult);
		...
		pv[0].vt = VT_UNKNOWN;
		punk->QueryInterface(IID_PPV_ARGS(&pv[0].punkVal));
...
HRESULT DoStatusText(IUnknownPtr punk, LPCWSTR lpszStatusText, int iPart)
{
	...
	if (g_pOnFunc[TE_OnStatusText]) {
		...
		pv = GetNewVARIANT(3);
		pv[2].vt = VT_UNKNOWN;
		punk->QueryInterface(IID_PPV_ARGS(&pv[2].punkVal));
		pv[1].vt = VT_BSTR;
...
LONGLONG DoHitTest(IUnknownPtr punk, POINT pt, UINT flags)
{
	if (g_pOnFunc[TE_OnHitTest]) {
		...
		pv[2].vt = VT_UNKNOWN;
		punk->QueryInterface(IID_PPV_ARGS(&pv[2].punkVal));
		CteMemory *pstPt = new CteMemory(2 * sizeof(int), NULL, VT_VARIANT, 1, L"POINT");
		::CopyMemory(pstPt->m_pc, &pt, 2 * sizeof(int));
		if SUCCEEDED(pstPt->QueryInterface(IID_PPV_ARGS(&pv[1].punkVal))) {
			pv[1].vt = VT_UNKNOWN;
		}
		pstPt->Release();
		pv[0].lVal = flags;
...
HRESULT DragSub(int nFunc, IUnknownPtr punk, CteFolderItems *pDragItems, DWORD grfKeyState, POINTL pt, DWORD *pdwEffect)
{
	HRESULT hr = E_FAIL;
	VARIANT vResult;
	VARIANTARG *pv;
	...
	if (g_pOnFunc[nFunc]) {
		try {
			if (InterlockedIncrement(&g_nProcDrag) < 10) {
				pv = GetNewVARIANT(5);
				if SUCCEEDED(punk->QueryInterface(IID_PPV_ARGS(&pv[4].punkVal))) {
					pv[4].vt = VT_UNKNOWN;
				}
				if SUCCEEDED(pDragItems->QueryInterface(IID_PPV_ARGS(&pv[3].punkVal))) {
					pv[3].vt = VT_UNKNOWN;
				}
				...
				pstPt = new CteMemory(2 * sizeof(int), NULL, VT_VARIANT, 1, L"POINT");
				::CopyMemory(pstPt->m_pc, &pt, 2 * sizeof(int));
				if SUCCEEDED(pstPt->QueryInterface(IID_PPV_ARGS(&pv[1].punkVal))) {
					pv[1].vt = VT_UNKNOWN;
				}
				...
				pstEffect = new CteMemory(sizeof(int), (char *)pdwEffect, VT_VARIANT, 1, L"DWORD");
				if SUCCEEDED(pstEffect->QueryInterface(IID_PPV_ARGS(&pv[0].punkVal))) {
					pv[0].vt = VT_UNKNOWN;
				}
				pstEffect->Release();
...
HRESULT DragLeaveSub(IUnknownPtr punk, CteFolderItems **ppDragItems)
{
	...
	return DoFunc(TE_OnDragLeave, punk, E_NOTIMPL);
}

HRESULT MessageSub(int nFunc, IUnknownPtr punk, MSG *pMsg, HRESULT hrDefault)
{
	VARIANT vResult;
	VARIANTARG *pv;
...
HRESULT MessageSubPt(int nFunc, IUnknownPtr punk, MSG *pMsg)
{
	VARIANT vResult;
	...
	if (g_pOnFunc[nFunc]) {
		...
		CteMemory *pMem = new CteMemory(2 * sizeof(int), NULL, VT_VARIANT, 1, L"POINT");
		pMem->SetLong(0, pMsg->pt.x);
		pMem->SetLong(1, pMsg->pt.y);
		if SUCCEEDED(pMem->QueryInterface(IID_PPV_ARGS(&pv[0].punkVal))) {
			pv[0].vt = VT_UNKNOWN;
		}
		pMem->Release();
				...
				if (ControlFromhwnd(&pdisp, pmse->hwnd) == S_OK) {
					try {
						if (InterlockedIncrement(&g_nProcMouse) == 1) {
							...
							if SUCCEEDED(pdisp->QueryInterface(IID_PPV_ARGS(&pv[6].punkVal))) {
								pv[6].vt = VT_UNKNOWN;
							}
							...
							pv[3].vt = VT_I4;
							CteMemory *pMem;
							pMem = new CteMemory(2 * sizeof(int), NULL, VT_VARIANT, 1, L"POINT");
							pMem->SetLong(0, pmse->pt.x);
							pMem->SetLong(1, pmse->pt.y);
							if SUCCEEDED(pMem->QueryInterface(IID_PPV_ARGS(&pv[2].punkVal))) {
								pv[2].vt = VT_UNKNOWN;
							}
							pMem->Release();
							...
							if (hrResult != S_OK && nTV >= 0) {
								if (wParam == WM_LBUTTONUP || wParam == WM_LBUTTONDBLCLK || wParam == WM_MBUTTONUP) {
									if (g_pSB[nTV]->m_pTV->m_pOleObject && ht.hItem) {
...
LRESULT CALLBACK TETVProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	if (msg == WM_NOTIFY) {
		...
		if (pnmhdr->code == NM_RCLICK) {		//This is necessary to call the WM_CONTEXTMENU
			try {
				...
				case TTN_GETDISPINFO:
					if (g_pOnFunc[TE_OnToolTip]) {
						...
						if (nTC >= 0) {
							...
							VariantInit(&pv[1]);
							pv[1].vt = VT_UNKNOWN;
							g_pTC[nTC]->QueryInterface(IID_PPV_ARGS(&pv[1].punkVal));
							VariantInit(&pv[0]);
							...
							::ZeroMemory(g_szText, MAX_PATH);
							lstrcpyn(g_szText, vText.bstrVal, MAX_PATH);
...
{
	try {
		for (int i = (sizeof(g_maps) / sizeof(*g_maps)) - 1; i >= 0; i--) {
			delete [] g_maps[i];
		}
...
void teCalcClientRect(int *param, LPRECT rc, LPRECT rcClient) 
{
	...
	else if (param[TE_Left] >= 0) {
		rc->left = param[TE_Left] / 0x4000 + rcClient->left;
	}
	else {
		rc->left = param[TE_Left] / 0x4000 + rcClient->right;
	}

	if (param[TE_Top] & 0x3fff) {
		rc->top = (param[TE_Top] * (rcClient->bottom - rcClient->top)) / 10000 + rcClient->top;
	}
	else if (param[TE_Top] >= 0) {
		rc->top = param[TE_Top] / 0x4000 + rcClient->top;
	}
	else {
		rc->top = param[TE_Top] / 0x4000 + rcClient->bottom;
	}
	if (param[TE_Width] & 0x3fff) {
		rc->right = param[TE_Width] * (rcClient->right - rcClient->left) / 10000 + rc->left;
	}
	else if (param[TE_Width] >= 0) {
		rc->right = param[TE_Width] / 0x4000 + rc->left;
	}
	else {
		rc->right = param[TE_Width] / 0x4000 + rcClient->right;
	}
	...
	else if (param[TE_Height] >= 0) {
		rc->bottom = param[TE_Height] / 0x4000 + rc->top;
	}
	else {
		rc->bottom = param[TE_Height] / 0x4000 + rcClient->bottom;
	}
...
BOOL CanClose(IUnknownPtr punk)
{
	return DoFunc(TE_OnClose, punk, S_OK) != S_FALSE;
}

VOID ArrangeTree(CteShellBrowser *pSB, LPRECT rc)//, int left, int top)TODO
{
	RECT rcTree = *rc;
	...
	if (pTV) {
		...
		if (pTV->m_pOleObject) {
			...
			pTV->m_pOleObject->QueryInterface(IID_PPV_ARGS(&pOleInPlaceObject));
	...
	switch (idEvent) {
		...
		case TET_Reload:
			if (g_bReload) {
				...
				szName[0] = '"';
				int i = GetModuleFileName(NULL, &szName[1], MAX_PATH);
				szName[i + 1] = '"';
				szName[i + 2] = 0;
				ShellExecute(hwnd, NULL, szName, NULL, NULL, SW_SHOWNORMAL);
			}
			break;
		case TET_Size2:
			if (g_nSize > 0) {
				...
				if (pTC) {
					...
					try {
						...
						if (g_pOnFunc[TE_OnArrange]) {
							...
							pv[0].vt = VT_UNKNOWN;
							CteMemory *pstR;
							pstR = new CteMemory(sizeof(RECT), (char *)&rc, VT_VARIANT, 1, L"RECT");
							pstR->QueryInterface(IID_PPV_ARGS(&pv[0].punkVal));
							pstR->Release();
							pTC->QueryInterface(IID_PPV_ARGS(&pv[1].punkVal));
						...
						if (!pTC->m_bEmpty && pTC->m_bVisible) {
							...
							if (nCount && rcTab.bottom) {
								...
								if (teSetRect(pTC->m_hwnd, 0, -pTC->m_si.nPos, rcTab.right, rcTab.bottom + pTC->m_si.nPos)) {
									ArrangeWindow();
								}
								if (pSB) {
					...
					int       nCmdShow)
{
	...
	PathAppend(szPath, L"kernel32.dll");
	...
	GetSystemDirectory(szPath, MAX_PATH);
	PathAppend(szPath, L"shell32.dll");
	g_hShell32 = LoadLibrary(szPath);
	if (g_hShell32) {
	...
	GetSystemDirectory(szPath, MAX_PATH);
	PathAppend(szPath, L"crypt32.dll");
	g_hCrypt32 = LoadLibrary(szPath);
	if (g_hCrypt32) {
	...
	GetVersionEx(&osInfo);
	//Multiple Launch
	GetModuleFileName(NULL, szPath, sizeof(szPath));//Executable Path
	for (int i = 0; szPath[i]; i++) {
		if (szPath[i] == '\\') {
			szPath[i] = '/';
		}
		szPath[i] = towupper(szPath[i]);
	}
	g_hMutex = CreateMutex(NULL, FALSE, szPath);
	if (WaitForSingleObject(g_hMutex, 0) == WAIT_TIMEOUT) {
		HWND hwndTE = FindWindow(WINDOW_CLASS, NULL);
		if (hwndTE) {
			COPYDATASTRUCT cd;
			cd.dwData = 0;
			cd.lpData = teGetCommandLine();
			cd.cbData = lstrlen((LPCWSTR)cd.lpData) * sizeof(WCHAR);
			DWORD_PTR dwResult;
			if (SendMessageTimeout(hwndTE, WM_COPYDATA, nCmdShow, LPARAM(&cd), SMTO_ABORTIFHUNG, 1000 * 30, &dwResult)) {
				if (dwResult == S_OK) {
	...
	CoCreateInstance(clsid, NULL, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&g_psha));
	//Get JScript Object
	CLSIDFromProgID(L"htmlfile", &clsid);
	if SUCCEEDED(CoCreateInstance(clsid, NULL, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&g_pHtmlDoc))) {
	...
	PathAppend(szPath, L"script\\index.html");

	//CTE
	g_pTE = new CTE();
	g_pTE->m_param[TE_CmdShow] = nCmdShow;
	g_pWebBrowser = new CteWebBrowser(g_hwndMain, (WCHAR *)&szPath);

	//Open Instead of Explorer
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
		...
		case WM_SIZE:
			...
			break;
		case WM_COPYDATA:
			if (!g_bReload && g_pOnFunc[TE_OnCopyData]) {
				VARIANTARG *pv;
				pv = GetNewVARIANT(4);
				COPYDATASTRUCT *pcd;
				pcd = (COPYDATASTRUCT *)lParam;
				if SUCCEEDED(g_pTE->QueryInterface(IID_PPV_ARGS(&pv[3].punkVal))) {
					pv[3].vt = VT_UNKNOWN;
				}
				pv[2].bstrVal = SysAllocStringByteLen((LPCSTR)pcd->lpData, pcd->cbData);
				pv[2].vt = VT_BSTR;
				SetVariantLL(&pv[1], wParam);
				SetVariantLL(&pv[0], pcd->dwData);
				VARIANT vResult;
				VariantInit(&vResult);
				if SUCCEEDED(Invoke4(g_pOnFunc[TE_OnCopyData], &vResult, 4, pv)) {
					return GetIntFromVariantClear(&vResult);
				}
			}
			return S_FALSE;
...
{
	for (int i = sizeof(m_pOnFunc) / sizeof(m_pOnFunc[0]); i >= 0; i--) {
		if (m_pOnFunc[i]) {
			try {
				m_pOnFunc[i]->Release();
				m_pOnFunc[i] = NULL;
...
HRESULT CteShellBrowser::Navigate3(FolderItem *pFolderItem, UINT wFlags, DWORD *param, CteShellBrowser **ppSB, FolderItems *pFolderItems)
{
	...
	m_pTabs->LockUpdate();
	try {
		hr = Navigate2(pFolderItem, wFlags, param, ppSB, pFolderItems, NULL);
	} catch (...) {
		hr = E_FAIL;
	}
...
HRESULT CteShellBrowser::Navigate2(FolderItem *pFolderItem, UINT wFlags, DWORD *param, CteShellBrowser **ppSB, FolderItems *pFolderItems, LPITEMIDLIST pidlFocus)
{
	RECT rc;
	LPITEMIDLIST pidl;
	IFolderView2 *pFV2;
	...
	if (!GetAbsPidl(&pidl, &pFolderItem1, pFolderItem, wFlags)) {
		return S_OK;
	}
	if (m_pidl && wFlags & SBSP_PARENT) {
		pidlFocus = ::ILFindLastID(m_pidl);
	}
	wFlags &= ~(SBSP_PARENT | SBSP_NAVIGATEBACK | SBSP_NAVIGATEFORWARD | SBSP_RELATIVE);

	if (wFlags & SBSP_NEWBROWSER || m_bEmpty) {
		CteShellBrowser *pSB;
		if (!m_bEmpty && wFlags & SBSP_NEWBROWSER) {
		...
		if (!m_bEmpty) {
			pSB->m_bEmpty = false;
			FolderItem *pFolderItem1 = pFolderItem;
			if (pFolderItem == NULL) {
				GetFolderItemFromPidl(&pFolderItem1, pidl);
			}
			hr = pSB->Navigate2(pFolderItem1, wFlags & (~SBSP_NEWBROWSER), param, ppSB, pFolderItems, pidlFocus);
			(pFolderItem == NULL) && pFolderItem1 && pFolderItem1->Release();
			::CoTaskMemFree(pidl);
			if (hr == S_OK && (wFlags & SBSP_ACTIVATE_NOFOCUS) == 0) {
	...
	if (param[SB_DoFunc]) {
		if (g_pOnFunc[TE_OnBeforeNavigate]) {
			...
			pv[3].vt = VT_UNKNOWN;
			pv[2].vt = VT_UNKNOWN;
			QueryInterface(IID_PPV_ARGS(&pv[3].punkVal));
			pMem = new CteMemory(4 * sizeof(int), (char *)&m_param[SB_ViewMode], VT_VARIANT, 1, L"FOLDERSETTINGS");
			pMem->QueryInterface(IID_PPV_ARGS(&pv[2].punkVal));
			pv[1].vt = VT_I4;
			pv[1].lVal = wFlags;
			if (pPreviusView) {
				if (pPrevius && SUCCEEDED(pPrevius->QueryInterface(IID_PPV_ARGS(&pv[0].punkVal)))) {		
					pv[0].vt = VT_UNKNOWN;
				}
			}
			m_bInit = true;
			...
			if (hr != S_OK && m_nUnload != 2) {
				if ((static_cast<DWORD>(hr) == E_ACCESSDENIED) && (wFlags & SBSP_NEWBROWSER) == 0) {
					try {
						hr = Navigate2(pFolderItem, wFlags | SBSP_NEWBROWSER, param, ppSB, pFolderItems, pidlFocus);
					} catch (...) {
						hr = E_ABORT;
					}
				}
...
BOOL CteShellBrowser::GetAbsPidl(LPITEMIDLIST *pidlOut, FolderItem **ppid, FolderItem *pid, UINT wFlags)
{
	...
	if (wFlags & SBSP_NAVIGATEBACK) {
		BOOL b = (m_nLogIndex > 0) ? GetIDListFromObjectEx(m_ppLog[--m_nLogIndex], pidlOut) : false;
		if (b) {
	...
	if (wFlags & SBSP_NAVIGATEFORWARD) {
		BOOL b = (m_nLogIndex < m_nLogCount - 1) ? GetIDListFromObjectEx(m_ppLog[++m_nLogIndex], pidlOut) : false;
		if (b) {
			m_ppLog[m_nLogIndex]->QueryInterface(IID_PPV_ARGS(ppid));
		}
		return b;
...
VOID CteShellBrowser::Error(int *pnDog)
{
	...
	if (SHGetPathFromIDList(m_pidl, pszPath)) {
		int nDriveType = DriveType(PathGetDriveNumber(pszPath));
		if (pszPath[0] == '\\' || nDriveType == DRIVE_REMOTE || nDriveType == DRIVE_NO_ROOT_DIR) {
			*pnDog = 1;
		}
	}
	else {
	...
	if (*pnDog == 1) {
		::CoTaskMemFree(m_pidl);
		SHGetSpecialFolderLocation(NULL, CSIDL_DESKTOP, &m_pidl);
	}
	m_pFolderItem && m_pFolderItem->Release();
...
VOID CteShellBrowser::GetVariantPath(FolderItem **ppFolderItem, FolderItems **ppFolderItems, VARIANT *pv)
{
	IDispatch *pdisp;
	if (GetDispatch(pv, &pdisp)) {
		if SUCCEEDED(pdisp->QueryInterface(IID_PPV_ARGS(ppFolderItems))) {
			VARIANT v;
	...
	LPITEMIDLIST pidl;
	GetPidlFromVariant(&pidl, pv);
	GetFolderItemFromPidl(ppFolderItem, pidl);
	::CoTaskMemFree(pidl);
}
...
STDMETHODIMP CteShellBrowser::OnViewWindowActive(IShellView *ppshv)
{
	...
	WCHAR szPath[MAX_PATH];
	if (SHGetPathFromIDList(m_pidl, (LPWSTR)&szPath)) {
...
STDMETHODIMP CteShellBrowser::IncludeObject(IShellView *ppshv, LPCITEMIDLIST pidl)
{
	if (m_pOnFunc[SB_OnIncludeObject]) {
		...
		pv = GetNewVARIANT(2);
		pv[1].vt = VT_UNKNOWN;
		QueryInterface(IID_PPV_ARGS(&pv[1].punkVal));
		LPITEMIDLIST FullID;
		FullID = ILCombine(m_pidl, pidl);
		if (GetFolderItemFromPidl((FolderItem **)&pv[0].punkVal, FullID)) {
			pv[0].vt = VT_UNKNOWN;
		}
...
STDMETHODIMP CteShellBrowser::Invoke(DISPID dispIdMember, REFIID riid, LCID lcid, WORD wFlags, DISPPARAMS *pDispParams, VARIANT *pVarResult, EXCEPINFO *pExcepInfo, UINT *puArgErr)
{
	...
	switch(dispIdMember) {
			...
			if (nArg >= 0) {
				...
				if (pVarResult) {
					...
					if (dispIdMember == 0x10000004) {
						...
						if SUCCEEDED(pSB->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
			...
			if (dispIdMember == 0x1000000B) {
				if (pVarResult) {
					...
					for (int i = 1; i <= m_nLogCount; i++) {
						m_ppLog[m_nLogCount - i]->QueryInterface(IID_PPV_ARGS(&v.punkVal));
						v.vt = VT_UNKNOWN;
						pFolderItems->ItemEx(-1, NULL, &v);
						VariantClear(&v);
					}
					pFolderItems->m_nIndex = m_nLogCount - 1 - m_nLogIndex;
					if SUCCEEDED(pFolderItems->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
						pVarResult->vt = VT_UNKNOWN;
					}
					pFolderItems->Release();
				}
		...
		case 0x10000020:	
			if (pVarResult) {
				if (m_pFolderItem && SUCCEEDED(m_pFolderItem->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal)))) {
					pVarResult->vt = VT_UNKNOWN;
				}
				else if (GetFolderItemFromPidl((FolderItem **)&pVarResult->punkVal, m_pidl)) {
					pVarResult->vt = VT_UNKNOWN;
				}
			}
			return S_OK;
		...
		case 0x10000024:
			if (pVarResult) {
				...
				if SUCCEEDED(get_Parent(&pdisp)) {
					pdisp->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal));
					pVarResult->vt = VT_UNKNOWN;
					pdisp->Release();
				}
			}
		...
		case 0x10000050:
			if (pVarResult) {
				if SUCCEEDED(m_pDSFV->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
					pVarResult->vt = VT_UNKNOWN;
				}
			}
			return S_OK;			
		...
		case 0x10000108:
			if (pVarResult) {
				...
				if (pCDT) {
					if SUCCEEDED(pCDT->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
						pVarResult->vt = VT_UNKNOWN;
					}
					pCDT->Release();
				}
		...
		//Items
		case 0x10000202:
			if (pVarResult && m_pShellView && m_bVisible) {
				IDataObject *pDataObj;
		...
		case 0x10000207:
			if (pVarResult) {
				...
				if SUCCEEDED(m_pShellView->GetItemObject(SVGIO_BACKGROUND, IID_PPV_ARGS(&pCM))) {
					...
					if SUCCEEDED(pCCM->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
						pVarResult->vt = VT_UNKNOWN;
					}
					pCCM->Release();
					pCM->Release();
		...
		case 0x10000210:
			if (pVarResult) {
				if SUCCEEDED(m_pTV->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
					pVarResult->vt = VT_UNKNOWN;
				}
			}
			return S_OK;
		...
		case 0x10000281:
			if (pVarResult) {
				FolderItem *pFolder = NULL;
				if (get_FocusedItem(&pFolder) == S_OK) {
					if SUCCEEDED(pFolder->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
						pVarResult->vt = VT_UNKNOWN;
					}
					pFolder->Release();
				}
		...
		case DISPID_VALUE:
			if (pVarResult) {
				if SUCCEEDED(QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
					pVarResult->vt = VT_UNKNOWN;
				}
			}
			return S_OK;
			...
			if (dispIdMember >= START_OnFunc && dispIdMember < START_OnFunc + Count_SBFunc) {
				if (nArg >= 0) {
					...
					if (GetDispatch(&pDispParams->rgvarg[nArg], &pdisp)) {
						pdisp->QueryInterface(IID_PPV_ARGS(&m_pOnFunc[dispIdMember - START_OnFunc]));
					}
				}
				if (pVarResult) {
					if (m_pOnFunc[dispIdMember - START_OnFunc]) {
						if SUCCEEDED(pVarResult->punkVal->QueryInterface(IID_PPV_ARGS(&m_pOnFunc[dispIdMember - START_OnFunc]))) {
							pVarResult->vt = VT_UNKNOWN;
						}
					}
				}
...
STDMETHODIMP CteShellBrowser::OnNavigationPending(PCIDLIST_ABSOLUTE pidlFolder)
{
	...
	PostMessage(g_hwndMain, TEM_BrowseObject, m_nSB, (LPARAM) ::ILClone(pidlFolder));
	return E_FAIL;
}
...
STDMETHODIMP CteShellBrowser::GetPaneState(REFEXPLORERPANE ep, EXPLORERPANESTATE *peps)
{
	if (g_pOnFunc[TE_OnGetPaneState]) {
		VARIANTARG *pv = GetNewVARIANT(3);
		CteMemory *pstEps;
		pstEps = new CteMemory(sizeof(int), (char *)peps, VT_VARIANT, 1, L"DWORD");
...
LPITEMIDLIST CteShellBrowser::GetNameSpace()
{
...
#endif
#ifdef _2000XP
	if SUCCEEDED(SetSort2(&bs[dir])) {
		if (dir && dir1) {
			SetSort2(&bs[dir]);
		}
		return;
	}
	SetViewMenu('I', &bs[dir]);
	if (dir && dir1) {
		SetViewMenu('I', &bs[dir]);
	}
#endif
...
#ifdef _2000XP
HRESULT CteShellBrowser::SetSort2(LPWSTR lpstr)
{
	HRESULT hr = E_FAIL;
	IShellFolderView *pSFV;
	if SUCCEEDED(m_pShellView->QueryInterface(IID_PPV_ARGS(&pSFV))) {
...
VOID CteShellBrowser::SetViewMenu(WCHAR wc, LPWSTR lpstr)
{
#ifdef _2000XP
	IContextMenu *pCM;
	if SUCCEEDED(m_pShellView->GetItemObject(SVGIO_BACKGROUND, IID_PPV_ARGS(&pCM))) {
		...
		try {
			if SUCCEEDED(pCM->QueryContextMenu(hMenu, 0, 1, 0x7FFF, CMF_DEFAULTONLY)) {
				...
				while (--i >= 0) {
					LPWSTR lpMenu = teGetMenuString(hMenu, i, TRUE);
					if (lpMenu) {
						int nPos = tePos('&', lpMenu);
						if (nPos > 0) {
							if (lpMenu[nPos + 1] != wc) {
								nPos = -1;
							}
						...
						if (nPos >= 0) {
							...
							int j = GetMenuItemCount(hMenu2);
							while (--j >= 0) {
								lpMenu = teGetMenuString(hMenu2, j, TRUE);
								if (lpMenu) {
									try {
										teMenuText(lpMenu);
										if (lstrcmpi(lpstr, lpMenu) == 0) {
											UINT uCommand = GetMenuItemID(hMenu2, j);
											if (uCommand) {
												...
												cmi.lpVerb = (LPCSTR)MAKEINTRESOURCE(uCommand - 1);
...
CTE::CTE()
{
	...
	VariantInit(&m_Data);
	for (int i = 1; i < sizeof(m_param)/ sizeof(m_param[0]); i++) {
		m_param[i] = 0;
	}
...
STDMETHODIMP CTE::Invoke(DISPID dispIdMember, REFIID riid, LCID lcid, WORD wFlags, DISPPARAMS *pDispParams, VARIANT *pVarResult, EXCEPINFO *pExcepInfo, UINT *puArgErr)
{
	...
	if (dispIdMember >= TE_METHOD && wFlags == DISPATCH_PROPERTYGET) {
		if (pVarResult) {
			CteDispatch *pClone = new CteDispatch(this);
			pClone->m_dispIdMember = dispIdMember;
			if SUCCEEDED(pClone->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
				pVarResult->vt = VT_UNKNOWN;
			}
			pClone->Release();
			return S_OK;
	...
	switch(dispIdMember) {
				...
				if (nArg >= 0) {
					...
					switch (nCtrl) {
						...
						case CTRL_TV:
							...
							if (pSB) {
								if (nCtrl == CTRL_TV) {
									if SUCCEEDED(pSB->m_pTV->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
										pVarResult->vt = VT_UNKNOWN;
									}
								}
								else if SUCCEEDED(pSB->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
									pVarResult->vt = VT_UNKNOWN;
								}
							}
							break;
						case CTRL_TC:
							...
							if (pTC) {
								if SUCCEEDED(pTC->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
									pVarResult->vt = VT_UNKNOWN;
								}
							}
							break;
						case CTRL_WB:
							if SUCCEEDED(g_pWebBrowser->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
								pVarResult->vt = VT_UNKNOWN;
							}
							break;
					}
				...
				if (nArg >= 0) {
					...
					switch (nCtrl) {
						...
						case CTRL_TV:
							...
							pMem = new CteMemory(nCount * sizeof(IDispatch *), NULL, VT_UNKNOWN, nCount, NULL);
						...
						case CTRL_TC:
							...
							break;
						case CTRL_WB:
							pMem = new CteMemory(sizeof(IDispatch *), NULL, VT_UNKNOWN, 1, NULL);
							ppDisp = (IDispatch **)pMem->m_pc;
							g_pWebBrowser->QueryInterface(IID_PPV_ARGS(&ppDisp[0]));
							break;
					}
					if (!pMem) {
						pMem = new CteMemory(0, NULL, VT_UNKNOWN, 0, NULL);
					}
					if SUCCEEDED(pMem->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
						pVarResult->vt = VT_UNKNOWN;
					}
					pMem->Release();
				}
		...
		case DISPID_WINDOWREGISTERED:
			if (g_pOnFunc[TE_OnWindowRegistered]) {
				...
				VARIANTARG *pv = GetNewVARIANT(1);
				pv[0].vt = VT_UNKNOWN;
				g_pTE->QueryInterface(IID_PPV_ARGS(&pv[0].punkVal));
				...
				if (pVarResult) {
					...
					if SUCCEEDED(teCLSIDFromProgID(vClass.bstrVal, &clsid)) {
						if (dispIdMember == TE_METHOD + 1010) {
							if SUCCEEDED(CoCreateInstance(clsid, NULL, CLSCTX_INPROC_SERVER | CLSCTX_LOCAL_SERVER, 
								IID_PPV_ARGS(&pVarResult->punkVal))) {
								pVarResult->vt = VT_UNKNOWN;
							}
						}
						else if (dispIdMember == TE_METHOD + 1020) {
							if SUCCEEDED(GetActiveObject(clsid, NULL, &pVarResult->punkVal)) {
								pVarResult->vt = VT_UNKNOWN;
							}
						}
					}
		...
		case 1030:
			if (pVarResult) {
				...
				if SUCCEEDED(pAPI->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
					pVarResult->vt = VT_UNKNOWN;
				}
				pAPI->Release();
			}
		...
		case 1131:
			if (pVarResult) {
				...
				if SUCCEEDED(pCD->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
					pVarResult->vt = VT_UNKNOWN;
				}
				pCD->Release();
			}
		...
		case 1132:
			if (pVarResult) {
				...
				if SUCCEEDED(pGB->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
					pVarResult->vt = VT_UNKNOWN;
				}
				pGB->Release();
			}
		...
		case TE_METHOD + 1136:
			if (pVarResult && nArg >= 0 && GetDispatch(&pDispParams->rgvarg[nArg], &pdisp)) {
				CteDispatch *pObj = new CteDispatch(pdisp);
				pObj->m_bVB = TRUE;
				pObj->m_dispIdMember = DISPID_UNKNOWN;
				if SUCCEEDED(pObj->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
					pVarResult->vt = VT_UNKNOWN;
				}
				pObj->Release();
			}
		...
		case 1200:
			if (pVarResult) {
				...
				if SUCCEEDED(pO->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
					pVarResult->vt = VT_UNKNOWN;
				}
				pO->Release();
			}
			...
			if (nArg >= 4) {
				...
				switch (HIWORD(i)) {
					case 3:	//Tab
						...
						pTC = GetNewTC();
						pTC->Init();
						for (i = 0; i <= nArg && i < (sizeof(pTC->m_param) / sizeof(pTC->m_param[0])); i++) {
			...
			if (nArg >= 0) {
				if (pVarResult) {
					...
					if SUCCEEDED(ControlFromhwnd(&pdisp, (HWND)GetLLFromVariant(&pDispParams->rgvarg[nArg]))) {
						if SUCCEEDED(pdisp->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
							pVarResult->vt = VT_UNKNOWN;
						}
						pdisp->Release();
					}
		...
		case TE_METHOD + 1080:
			if (::InterlockedIncrement(&g_nLockUpdate) == 1) {
				SendMessage(g_hBrowser, WM_SETREDRAW, FALSE, 0);
			}
			return S_OK;
		//UnlockUpdate
		case TE_METHOD + 1090:
			if (::InterlockedDecrement(&g_nLockUpdate) <= 0) {
				g_nLockUpdate = 0;
				SendMessage(g_hBrowser, WM_SETREDRAW, TRUE, 0);
				RedrawWindow(g_hBrowser, NULL, 0, RDW_INVALIDATE | RDW_ALLCHILDREN);
		...
		default:
			if (dispIdMember >= START_OnFunc && dispIdMember < START_OnFunc + Count_OnFunc) {
				if (nArg >= 0) {
					...
					if (GetDispatch(&pDispParams->rgvarg[nArg], &pdisp)) {
						pdisp->QueryInterface(IID_PPV_ARGS(&g_pOnFunc[dispIdMember - START_OnFunc]));
					}
				}
				if (pVarResult) {
					if (g_pOnFunc[dispIdMember - START_OnFunc]) {
						if SUCCEEDED(g_pOnFunc[dispIdMember - START_OnFunc]->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
							pVarResult->vt = VT_UNKNOWN;
						}
					}
				}
...
/*		case DISPID_COMMANDSTATECHANGE:
		...
		case DISPID_DOCUMENTCOMPLETE:
			...
			if SUCCEEDED(m_pWebBrowser->QueryInterface(IID_PPV_ARGS(&pOleWindow))) {
				pOleWindow->GetWindow(&g_hBrowser);
				pOleWindow->Release();
			}
			if (g_bInit) {
				g_bInit = false;
		...
		case DISPID_NEWWINDOW3:
			if (g_pOnFunc[TE_OnNewWindow]) {
				...
				pv[3].vt = VT_UNKNOWN;
				g_pWebBrowser->QueryInterface(IID_PPV_ARGS(&pv[3].punkVal));
				for (int i = 2; i >= 0; i--) {
...
void CteWebBrowser::Close()
{
	...
	if (m_pWebBrowser) {
		...
		for (int i = 0; i< 3; i++) {	//error prevention at the end
...
VOID CteTabs::CreateTC()
{
	...
	if ((dwStyle & (TCS_BUTTONS | TCS_SCROLLOPPOSITE)) == (TCS_BUTTONS | TCS_SCROLLOPPOSITE)) {
	...
	switch(dispIdMember) {
		...
		case 7:	
			if (nArg >= 1) {
				...
				CteShellBrowser *pSB;
				if (GetDispatch(&pDispParams->rgvarg[nArg], &pdisp)) {
					if SUCCEEDED(pdisp->QueryInterface(g_ClsIdSB, (LPVOID *)&pSB)) {
				...
				if (nArg >= 2) {
					if (GetDispatch(&pDispParams->rgvarg[nArg - 2], &pdisp)) {
						CteTabs *pTC;
						if SUCCEEDED(pdisp->QueryInterface(g_ClsIdTC, (LPVOID *)&pTC)) {
							Move(nSrc, nDest, pTC);
							pTC->Release();
							return S_OK;
						}
		...
		case DISPID_NEWENUM:
			if (pVarResult) {
				pVarResult->punkVal = reinterpret_cast<IUnknown *>(new CteDispatch(this));
				pVarResult->vt = VT_UNKNOWN;
				return S_OK;
			}
			return E_FAIL;
				...
				if (dispIdMember == TE_OFFSET + TE_Flags) {
					...
					if ((dwStyle ^ m_param[TE_Flags]) & 0x7fff) {
						if ((dwStyle ^ m_param[TE_Flags]) & (TCS_SCROLLOPPOSITE | TCS_BUTTONS | TCS_TOOLTIPS)) {
							...
							SetWindowLong(m_hwnd, GWL_STYLE,  WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | TCS_FOCUSNEVER | m_param[TE_Flags]);
						}
					}
				}
...
/*					if (m_hwnd) {
				if (dispIdMember == TE_OFFSET + TE_Flags) {
					m_param[TE_Flags] = GetWindowLong(m_hwnd, GWL_STYLE);
				}
			}*/
...
CteShellBrowser* CteTabs::GetShellBrowser(int nPage)
{
	...
	if (tcItem.lParam < sizeof(g_pSB) / sizeof(g_pSB[0])) {
...
VOID CteFolderItems::ItemEx(int nIndex, VARIANT *pVarResult, VARIANT *pVarNew)
{
	...
	if (pVarNew) {
		...
		if (!m_oFolderItems) {
			...
			for (v1.lVal = 0; v1.lVal < nCount; v1.lVal++) {
				if (Item(v1, &pid) == S_OK) {
					pid->QueryInterface(IID_PPV_ARGS(&v2.punkVal));
					v2.vt = VT_UNKNOWN;
		...
		if (m_oFolderItems) {
			...
			pv[0].punkVal = NULL;
			IDispatch *pdisp;
			if (GetDispatch(pVarNew, &pdisp)) {
				pdisp->QueryInterface(g_ClsIdFI, (LPVOID *)&pv[0].punkVal);
			}
			if (pv[0].punkVal == NULL) {
				pv[0].punkVal = reinterpret_cast<IUnknown *>(new CteFolderItem(pVarNew));
			}
			pv[0].vt = VT_UNKNOWN;
	...
	if (pVarResult) {
		pVarResult->vt = VT_UNKNOWN;
		Item(v, (FolderItem **)&pVarResult->punkVal);
	}
}
...
STDMETHODIMP CteFolderItems::Invoke(DISPID dispIdMember, REFIID riid, LCID lcid, WORD wFlags, DISPPARAMS *pDispParams, VARIANT *pVarResult, EXCEPINFO *pExcepInfo, UINT *puArgErr)
{
	...
	switch(dispIdMember) {
		...
		case DISPID_NEWENUM:
			if (pVarResult) {
				if SUCCEEDED(_NewEnum(&pVarResult->punkVal)) {
					pVarResult->vt = VT_UNKNOWN;
				}
				return S_OK;
			}
		...
		case 0x10000002:
			if (pVarResult) {
				CteMemory *pMem = new CteMemory(sizeof(int), (char *)&m_dwEffect, VT_VARIANT, 1, L"DWORD");
				if SUCCEEDED(pMem->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
					pVarResult->vt = VT_UNKNOWN;
				}
				pMem->Release();
...
STDMETHODIMP CteFolderItems::get_Count(long *plCount)
{
	...
	if (m_oFolderItems) {
		...
		VariantClear(&v);
		DISPID dispid;
		LPOLESTR sz = L"length";
		m_oFolderItems->GetIDsOfNames(IID_NULL, &sz, 1, LOCALE_USER_DEFAULT, &dispid);
		HRESULT hr = Invoke5(m_oFolderItems, dispid, DISPATCH_PROPERTYGET, &v, 0, NULL);
	...
	*ppid = NULL;
	...
	if (i >= 0) {
		...
		if (m_oFolderItems) {
			...
			DISPID dispid;
			m_oFolderItems->GetIDsOfNames(IID_NULL, &v.bstrVal, 1, LOCALE_USER_DEFAULT, &dispid);
			Invoke5(m_oFolderItems, dispid, DISPATCH_PROPERTYGET, &v, 0, NULL);
			IDispatch *pdisp;
			if (GetDispatch(&v, &pdisp)) {
				HRESULT hr = pdisp->QueryInterface(IID_PPV_ARGS(ppid));
				VariantClear(&v);
				return hr;
			}
		}
...
STDMETHODIMP CteFolderItems::_NewEnum(IUnknown **ppunk)
{
	...
	*ppunk = reinterpret_cast<IUnknown *>(new CteDispatch(reinterpret_cast<IDispatch *>(this)));
	return S_OK;
}
	...
	if (pformatetcIn->cfFormat == CF_UNICODETEXT) {
		...
		try {
			if (g_pOnFunc[TE_OnClipboardText]) {
				...
				VARIANTARG *pv = GetNewVARIANT(1);
				pv[0].vt = VT_UNKNOWN;
				QueryInterface(IID_PPV_ARGS(&pv[0].punkVal));
	...
	if (dwDirection == DATADIR_GET) {
		...
		if (m_pDataObj) {
			...
			if SUCCEEDED(m_pDataObj->EnumFormatEtc(DATADIR_GET, &penumFormatEtc)) {
				while (nFormat < MAX_FORMATS - (sizeof(teformats) / sizeof(teformats[0])) && penumFormatEtc->Next(1, &formats[nFormat], NULL) == S_OK) {
		...
		AdjustIDListEx();
		int nMax = sizeof(teformats) / sizeof(teformats[0]);
...
STDMETHODIMP CteMemory::Invoke(DISPID dispIdMember, REFIID riid, LCID lcid, WORD wFlags, DISPPARAMS *pDispParams, VARIANT *pVarResult, EXCEPINFO *pExcepInfo, UINT *puArgErr)
{
	...
	switch(dispIdMember) {
		...
		case 8:
			if (pVarResult) {
				if (m_nSize) {
					CteMemory *mem = new CteMemory(m_nSize, NULL, m_nMode, m_nCount, m_bsStruct);
					::CopyMemory(mem->m_pc, m_pc, m_nSize);
					if SUCCEEDED(mem->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
						pVarResult->vt = VT_UNKNOWN;
					}
					mem->Release();
				}
...
STDMETHODIMP CteMemory::GetDispID(BSTR bstrName, DWORD grfdex, DISPID *pid)
{
	...
	if (lstrcmpi(m_bsStruct, L"BITMAP") == 0) {
		...
		if (lstrcmpi(bstrName, L"bmPlanes") == 0) {
			*pid = (VT_I2 << TE_VT) + (DWORD)(UINT_PTR)&p->bmPlanes;
			return S_OK;
		}
		if (lstrcmpi(bstrName, L"bmBitsPixel") == 0) {
			*pid = (VT_I2 << TE_VT) + (DWORD)(UINT_PTR)&p->bmBitsPixel;
			return S_OK;
		}
		...
		if (pVarResult) {
			switch (m_nMode) {
				case VT_BSTR:
					BSTR *lpBSTRdata;
					lpBSTRdata = (BSTR*)m_pc;
					pVarResult->bstrVal = SysAllocString(lpBSTRdata[i]);
					pVarResult->vt = VT_BSTR;
					break;
				case VT_UNKNOWN:
				case VT_DISPATCH:
					...
					ppUnk = (IUnknown **)m_pc;
					if (ppUnk[i]) {
						if SUCCEEDED(ppUnk[i]->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
							pVarResult->vt = VT_UNKNOWN;
						}
					}
					break;
					...
					if (m_nCount) {
						...
						if (nSize) {
							if (nSize == 1) {//BYTE/char
								...
								nSize -= nOffset;
								CteMemory *mem = new CteMemory(nSize, m_pc + nOffset + (nSize * i), m_nMode, 1, GetSubStruct(m_bsStruct));
								if SUCCEEDED(mem->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
									pVarResult->vt = VT_UNKNOWN;
								}
								mem->Release();
	...
	if (pVarResult->vt & VT_ARRAY) {
		...
		if (nSize) {
			...
			if (nLen > 0) {
				...
				if (psa) {
					::CopyMemory(psa->pvData, &m_pc[nIndex], nLen * nSize);
...
/*			if (nLen < 0) {
		...
		case VT_CY:
			CteMemory *pstPt;
			pstPt = new CteMemory(sizeof(POINT), NULL, VT_VARIANT, 1, L"POINT");
			::CopyMemory(pstPt->m_pc, &m_pc[nIndex], 2 * sizeof(int));	
			if SUCCEEDED(pstPt->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
				pVarResult->vt = VT_UNKNOWN;
			}
			pstPt->Release();
			break;
		case VT_CARRAY:
			CteMemory *pstR;
			pstR = new CteMemory(sizeof(RECT), NULL, VT_VARIANT, 1, L"RECT");
			::CopyMemory(pstR->m_pc, &m_pc[nIndex], 4 * sizeof(int));	
			if SUCCEEDED(pstR->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
				pVarResult->vt = VT_UNKNOWN;
			}
			pstR->Release();
			break;
...
VOID CteMemory::Write(int nIndex, int nLen, VARTYPE vt, VARIANT *pv)
{
	if (pv->vt & VT_ARRAY) {
		...
		if (nSize) {
			...
			if (nLen > 0) {
				::CopyMemory(&m_pc[nIndex], pv->parray->pvData, nLen * nSize);
			}
		}
		return;
	}
		...
		case 2:	
			if (nArg >= 7) {
				if (g_pOnFunc[TE_OnInvokeCommand]) {
					...
					pv[nArg + 1].vt = VT_UNKNOWN;
					QueryInterface(IID_PPV_ARGS(&pv[nArg + 1].punkVal));
...
STDMETHODIMP CteDropTarget::Invoke(DISPID dispIdMember, REFIID riid, LCID lcid, WORD wFlags, DISPPARAMS *pDispParams, VARIANT *pVarResult, EXCEPINFO *pExcepInfo, UINT *puArgErr)
{
	...
	switch(dispIdMember) {
		...
		case 6:	
			if (pVarResult) {
				if (GetFolderItemFromPidl((FolderItem **)&pVarResult->punkVal, m_pidl)) {
					pVarResult->vt = VT_UNKNOWN;
				}
			}
			return S_OK;
...
void CteTreeView::Close()
{
	...
	if (m_pOleObject) {
		RECT rc;
		SetRectEmpty(&rc);
		m_pOleObject->DoVerb(OLEIVERB_HIDE, NULL, NULL, 0, g_hwndMain, &rc);
...
//		false &&
...
#endif
#ifdef _2000XP
	HRESULT hr = CoCreateInstance(CLSID_ShellShellNameSpace, NULL, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&m_pOleObject));
	if FAILED(hr) {
		hr = CoCreateInstance(CLSID_ShellNameSpace, NULL, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&m_pOleObject));
	}
	if FAILED(hr) {
		return FALSE;
	}
	...
	if FAILED(m_pOleObject->DoVerb(OLEIVERB_INPLACEACTIVATE, NULL, static_cast<IOleClientSite *>(this), 0, g_hwndMain, &rc)) {
		return FALSE;
	}

	IOleWindow *pOleWindow;
	if SUCCEEDED(m_pOleObject->QueryInterface(IID_PPV_ARGS(&pOleWindow))) {
		if (pOleWindow->GetWindow(&m_hwnd) == S_OK) {
			IShellNameSpace *pSNS;
			if SUCCEEDED(m_pOleObject->QueryInterface(IID_PPV_ARGS(&pSNS))) {
				pSNS->put_Flags(m_pFV->m_param[SB_TreeFlags] & NSTCS_FAVORITESMODE);
				HWND hwnd = FindTreeWindow(m_hwnd);
				if (hwnd) {
...
#endif
#ifdef _2000XP
	else if (m_pOleObject && IsEqualIID(riid, IID_IOleObject)) {
		return m_pOleObject->QueryInterface(riid, ppvObject);
	}
	else if (m_pOleObject && IsEqualIID(riid, IID_IShellNameSpace)) {
		return m_pOleObject->QueryInterface(riid, ppvObject);
	}
#endif
	else {
...
STDMETHODIMP CteTreeView::Invoke(DISPID dispIdMember, REFIID riid, LCID lcid, WORD wFlags, DISPPARAMS *pDispParams, VARIANT *pVarResult, EXCEPINFO *pExcepInfo, UINT *puArgErr)
{
	...
	if (dispIdMember >= 0x10000101 && dispIdMember < TE_OFFSET) {
		if (m_pFV->m_param[SB_TreeAlign] & TE_TV_View) {
			if (!m_pNameSpaceTreeControl && !m_pOleObject) {
				Create();
			}
			if (m_bSetRoot && (m_pNameSpaceTreeControl || m_pOleObject)) {
				SetRoot();
			}
		}
	...
	switch(dispIdMember) {
		...
		case 0x10000007:
			if (pVarResult) {
				if SUCCEEDED(m_pFV->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
					pVarResult->vt = VT_UNKNOWN;
				}
			}
			return S_OK;
...
#ifdef _2000XP
		if (m_pOleObject) {
			for (int i = (sizeof(methodTV) / sizeof(methodTV[0])) - 1; i >= 0; i--) {
...
STDMETHODIMP CteTreeView::OnItemClick(IShellItem *psi, NSTCEHITTEST nstceHitTest, NSTCSTYLE nsctsFlags)
{
	if (g_pOnFunc[TE_OnItemClick]) {
		...
		if (m_pNameSpaceTreeControl) {
			...
			IDispatch *pdisp;
			if (getSelected(&pdisp) == S_OK) {
				pdisp->QueryInterface(IID_PPV_ARGS(&pv[2].punkVal));
				pv[2].vt = VT_UNKNOWN;
				pdisp->Release();
			}
		}
...
//Init IShellNameSpace

VOID CteTreeView::QuickActivate()
{
	HRESULT        hr;
...
BOOL CteTreeView::SetEventSink()
{
	...
	hr = pConnectionPointContainer->FindConnectionPoint(IID_IShellNameSpace, &pConnectionPoint);
	pConnectionPointContainer->Release();
	if (FAILED(hr))	
		return FALSE;

	hr = pConnectionPoint->Advise(static_cast<IDispatch *>(this), &m_dwEventCookie);
...
{
	if (m_pOleObject && !(m_pFV->m_param[SB_EnumFlags] & SHCONTF_NONFOLDERS)) {
		HWND hwnd = FindTreeWindow(m_hwnd);
		TVITEM item;
		...
		if (item.hItem) {
			...
			if SUCCEEDED(m_pOleObject->QueryInterface(IID_PPV_ARGS(&pSNS))) {
				IDispatch *pItem;
				if SUCCEEDED(pSNS->get_SelectedItem(&pItem)) {
					LPITEMIDLIST pidl;
...
HRESULT CteTreeView::NSInvoke(LPWSTR lpVerb, WORD wFlags, DISPPARAMS *pDispParams, VARIANT *pVarResult)
{
	...
	if (m_pOleObject) {
		IDispatch *pDispatch;
		if SUCCEEDED(m_pOleObject->QueryInterface(IID_PPV_ARGS(&pDispatch))) {
			DISPID dispid;
...
HRESULT CteTreeView::SetRoot()
{
	...
	if (hr != S_OK && m_pOleObject) {
		...
		IShellNameSpace *pSNS;
		if SUCCEEDED(m_pOleObject->QueryInterface(IID_PPV_ARGS(&pSNS))) {
			pSNS->put_EnumOptions(m_pFV->m_param[SB_EnumFlags]);
			...
			if (hr != S_OK) {
				if (m_pFV->m_vRoot.vt == VT_BSTR) {
					hr = pSNS->SetRoot(m_pFV->m_vRoot.bstrVal);
					if (hr != S_OK) {
						VARIANT v;
					...
					hr = pSNS->put_Root(m_pFV->m_vRoot);
					pSNS->SetRoot(NULL);
				}
			}
			pSNS->Release();
...
LPITEMIDLIST CteFolderItem::GetPidl()
{
	if (m_v.vt != VT_EMPTY) {
		...
		if (!m_dwTick || GetTickCount() - m_dwTick > 3000) {
	...
	switch(dispIdMember) {
		...
		case 5:
			if (pVarResult) {
				...
				if SUCCEEDED(hr) {
					hr = pdisp->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal));
					if SUCCEEDED(hr) {
						pVarResult->vt = VT_UNKNOWN;
					}
				}
		...
		case 14:
			if (pVarResult) {
				hr = Verbs((FolderItemVerbs **)&pVarResult->punkVal);
				if SUCCEEDED(hr) {
					pVarResult->vt = VT_UNKNOWN;
				}
			}
		...
		case DISPID_VALUE:
			if (pVarResult) {
				pVarResult->vt = VT_UNKNOWN;
				QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal));
			}
			return S_OK;
	}
	return DISP_E_MEMBERNOTFOUND;
					...
					if SUCCEEDED(CreateStreamOnHGlobal(NULL, TRUE, (LPSTREAM*)&oStream)) {
						if (m_pImage->Save(oStream, &encoderClsid) == 0) {
							...
							if SUCCEEDED(oStream->Read(pBuff, (ULONG)ulnSize.QuadPart, &ulBytesRead)) {
								...
								if (dwSize > 0) {
									pVarResult->vt = VT_BSTR;
									pVarResult->bstrVal = SysAllocStringLen(szHead, nDest + dwSize - 1);
...
STDMETHODIMP CteWindowsAPI::Invoke(DISPID dispIdMember, REFIID riid, LCID lcid, WORD wFlags, DISPPARAMS *pDispParams, VARIANT *pVarResult, EXCEPINFO *pExcepInfo, UINT *puArgErr)
{
		...
		if (dispIdMember >= 50000) {//string
			if (pVarResult) {
				...
				switch(dispIdMember) {
					...
					case 50015:
						if (nArg >= 0) {
							pszName = new WCHAR[MAX_PATH + 1];
							int i = GetClassName((HWND)param[0], pszName, MAX_PATH);
							pszName[i + 1] = 0;
						}
						break;
					case 50145:
					...
					case 50025:
						if (nArg >= 0) {
							pszName = new WCHAR[MAX_PATH + 1];
							int i = GetModuleFileName((HMODULE)param[0], pszName, MAX_PATH);
							pszName[i] = 0;
						}
						break;
					case 50035:
					...
					case 50065: //GetDisplayNameOf
						if (nArg >= 1) {
							...
							if (i & (SHGDN_FORPARSING | SHGDN_FORPARSINGEX) && (GetDispatch(&pDispParams->rgvarg[nArg], &pdisp))) {
								CteFolderItem *pFI = NULL;
								if SUCCEEDED(pdisp->QueryInterface(g_ClsIdFI, (LPVOID *)&pFI)) {
					...
					case 50085:
						if (nArg >= 1) {
							pszName = new WCHAR[MAX_PATH];
							UINT i = DragQueryFile((HDROP)param[0], (UINT)param[1], pszName, MAX_PATH);
							if (param[1] & ~MAXINT) {
								if (pVarResult) {
									pVarResult->vt = VT_I4;
									pVarResult->lVal = i;
			...
			switch(dispIdMember) {
				...
				case 30212://CRC32
					if (nArg >= 0) {
						...
						GetpDataFromVariant(&pc, &nLen, &pDispParams->rgvarg[nArg]);
						c = ((nArg >= 1) ? (UINT)param[1] : 0) ^ MAXUINT;
						for (i = 0; i < nLen; i++) {
				...
				case 35002://TrackPopupMenuEx
					if (nArg >= 5) {
						if (nArg >= 6) {
							if (GetDispatch(&pDispParams->rgvarg[nArg - 6], &pdisp)) {
								pdisp->QueryInterface(g_ClsIdContextMenu, (LPVOID *)&g_pCCM);
							}
						}
		...
		if (nArg >= 0) {
			...
			switch(dispIdMember) {
				case 6000://ILCreateFromPath
					...
					if (pidl) {
						FolderItem *ppid;
						if (GetFolderItemFromPidl(&ppid, pidl)) {
							if SUCCEEDED(ppid->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
								pVarResult->vt = VT_UNKNOWN;
							}
							ppid && ppid->Release();
						}
				...
				case 6010://GetProcObject
					...
					break;
				case 6001:
					*pbResult = SetCurrentDirectory(v.bstrVal);
					break;
				case 6002:
				...
				case 6035:
					if (v.bstrVal) {
						pszResult = new WCHAR[MAX_PATH + 1];
	...
	switch(dispIdMember) {
		...
		case 1040:	
			if (nArg >= 0) {
				if (pVarResult) {
					...
					pMem = new CteMemory(i * nCount + GetOffsetOfStruct(sz), pc, VT_VARIANT, nCount, sz);
					if (bs) {
		...
		case 2073:
			...
			break;
/*		//FindBrowserWindow(document) Did not work on modeless dialog
		case 2083:
			if (nArg >= 0) {
				if (pDispParams->rgvarg[nArg].vt == VT_DISPATCH) {
					IServiceProvider *pSP1;
					if (pDispParams->rgvarg[nArg].pdispVal->QueryInterface(IID_PPV_ARGS(&pSP1)) == S_OK) {
						IServiceProvider *pSP2;
						if (pSP1->QueryService(SID_STopLevelBrowser, IID_IServiceProvider, reinterpret_cast<void**>(&pSP2)) == S_OK) {
							SHDocVw::IWebBrowser *pIE;
							if (pSP1->QueryService(SID_SWebBrowserApp, IID_IWebBrowser2, reinterpret_cast<void **>(&pIE)) == S_OK) {
								IOleWindow * pOleWindow;
								if SUCCEEDED(pIE->QueryInterface(IID_PPV_ARGS(&pOleWindow))) {
									pOleWindow->GetWindow((HWND *)phResult);
									pOleWindow->Release();
								}
								pIE->Release();
							}
				...
				if (nArg >= 0) {
					...
					if (i > 0) {
						...
						if (pMem) {
							...
							pMem->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal));
							pVarResult->vt = VT_UNKNOWN;
							pMem->Release();
						}
					}
			...
			if (pVarResult && nArg >= 0) {
				...
				if (GetPidlFromVariant(&pidl, &pDispParams->rgvarg[nArg])) {
					...
					if (GetFolderItemFromPidl(&pFI, pidl)) {
						if SUCCEEDED(pFI->QueryInterface(IID_PPV_ARGS(&pVarResult->punkVal))) {
							pVarResult->vt = VT_UNKNOWN;
						}
						pFI && pFI->Release();
					}
			...
			if (nArg >= 1 && pVarResult) {
				...
				if (pszData && pszFormat) {
					...
					while (WCHAR wc = pszFormat[nPos]) {
						...
						if (StrChrIW(L"s", wc)) {//String
							pVarResult->vt = VT_BSTR;
							swscanf_s(pszData, pszFormat, &pVarResult->bstrVal);
							return S_OK;
						}
					}
				}
...
CteDispatch::CteDispatch(IDispatch *pDispatch)
{
	m_cRef = 1;
...
{
	if (ppEnum) {
		*ppEnum = new CteDispatch(m_pDispatch);
		return S_OK;
	}
	return E_POINTER;
}
