...
{
	ULONG chEaten = 0;
	return g_pPUI->ParsePropertyName(pszString, &pkey->fmtid, &pkey->pid, &chEaten);
}
#endif
				...
				if (ControlFromhwnd(&pdisp, pmse->hwnd) == S_OK) {
					try {
						if (InterlockedIncrement(&g_nProcMouse) == 1) {
							CteTreeView *pTV = TVfromhwnd(pmse->hwnd, false);
							TVHITTESTINFO ht;
							if (pTV) {
								ht.pt.x = pmse->pt.x;
								ht.pt.y = pmse->pt.y;
				...
				if (InterlockedIncrement(&g_nProcFV) == 1) {
					switch (msg) {
						...
						case SB_SETTEXTA:
							...
							lResult = DoStatusText(pSB, const_cast<LPCWSTR>(lpszW), static_cast<int>(wParam));
							delete []lpszW;
							break;
						case SB_SETTEXT:
							lResult = DoStatusText(pSB, (LPCWSTR)lParam, static_cast<int>(wParam));
							break;
						case WM_COMMAND:
							if (g_pOnFunc[TE_OnCommand]) {
								MSG msg1;
...
VOID teSetTreeWidth(HWND hwnd, LPARAM lParam)
{
	CteTabs *pTC = TCfromhwnd(hwnd);
	if (pTC) {
		CteShellBrowser	*pSB = pTC->GetShellBrowser(pTC->m_nIndex);
		if (pSB) {
			int nWidth = pSB->m_param[SB_TreeWidth] + SHORT(lParam) - g_x;
			RECT rc;
			GetWindowRect(hwnd, &rc);
...
LRESULT CALLBACK TEBTProc2(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	LRESULT Result = 1;
	switch (msg) {
		case WM_MOUSEMOVE:
	...
	return Result ? CallWindowProc(g_DefBTProc2, hwnd, msg, wParam, lParam) : 0;
}
		...
		case TCM_SETITEM:
			...
			if (pTC) {
				if (pTC->m_param[TE_Flags] & TCS_FIXEDWIDTH) {
					...
					CallWindowProc(g_DefTCProc, pTC->m_hwnd, TCM_SETITEMSIZE, 0, MAKELPARAM(pTC->m_param[TC_TabWidth], pTC->m_param[TC_TabHeight]));
				}
			}
			break;
		case WM_SETFOCUS:
					...
					try {
						...
						if (!pTC->m_bEmpty && pTC->m_bVisible) {
							...
							if (true) {
								...
								if (pSB) {
									if (pSB->m_param[SB_TreeAlign]) {
										ArrangeTree(pSB, &rc);
									}
									if (pSB->m_bVisible) {
										teSetParent(pSB->m_pTV->m_hwnd, pSB->m_pTabs->m_hwndStatic);
										ShowWindow(pSB->m_pTV->m_hwnd, pSB->m_param[SB_TreeAlign] ? SW_SHOW : SW_HIDE);
									}
									else {
										pSB->Show(TRUE);
									}
					...
					int       nCmdShow)
{
	...
	if (g_hPropsys) {
		...
		if SUCCEEDED(CoCreateInstance(CLSID_PropertiesUI, NULL, CLSCTX_INPROC, IID_PPV_ARGS(&g_pPUI))) {
		...
		case WM_POWERBROADCAST:
			if (wParam == PBT_APMRESUMEAUTOMATIC) {
				for (int i = MAX_FV; i--;) {
					...
					if (pSB) {
						if (!pSB->m_bEmpty && pSB->m_nUnload == 0 && pSB->m_pidl && pSB->m_pFolderItem) {
							VARIANT v;
							::VariantInit(&v);
							v.vt = VT_BSTR;
							if SUCCEEDED(GetDisplayNameFromPidl(&v.bstrVal, pSB->m_pidl, SHGDN_FORPARSING)) {
								if (PathIsNetworkPath(v.bstrVal)) {
									pSB->m_pFolderItem->Release();
									pSB->m_pFolderItem = new CteFolderItem(&v);
									::CoTaskMemFree(pSB->m_pidl);
									pSB->m_pidl = NULL;
									pSB->m_nUnload = 9;
									if (pSB->m_bVisible) {
										pSB->Show(TRUE);
...
HRESULT CteShellBrowser::Navigate2(FolderItem *pFolderItem, UINT wFlags, DWORD *param, FolderItems *pFolderItems, LPITEMIDLIST pidlPrevius)
{
	...
	m_bIconSize = FALSE;
	if (param[SB_DoFunc]) {
		hr = OnBeforeNavigate(pPrevius, wFlags);
		if (hr == E_ACCESSDENIED && ILIsEqual(m_pidl, pidlPrevius)) {
			hr = S_OK;
		}
...
VOID CteShellBrowser::Error(int *pnDog)
{
	...
	BSTR bs;
	if SUCCEEDED(GetDisplayNameFromPidl(&bs, m_pidl, SHGDN_FORPARSING)) {
		...
		case 0x20000007:
			if (nArg >= 3) {
				...
				if (pFolderItem) {
					...
					Navigate2(pFolderItem, GetIntFromVariant(&pDispParams->rgvarg[nArg - 1]), param, pFolderItems, pidl);
					if (m_bVisible) {
						Show(TRUE);
					}
				}
...
STDMETHODIMP CteShellBrowser::OnNavigationPending(PCIDLIST_ABSOLUTE pidlFolder)
{
	...
	HRESULT hr = OnBeforeNavigate(pPrevius, SBSP_SAMEBROWSER | SBSP_ABSOLUTE);
	if FAILED(hr) {
		if (hr == E_ACCESSDENIED) {
			BrowseObject(pidlFolder, SBSP_NEWBROWSER | SBSP_ABSOLUTE);
		}
		m_pFolderItem && m_pFolderItem->Release();
		::CoTaskMemFree(m_pidl);
		m_pidl = pidlPrevius;
...
#ifdef _VISTA7
	if (m_pExplorerBrowser) {
		GetPidlFromSV(&m_pidl, m_pShellView);
...
#endif
	m_bIconSize = TRUE;
	if (m_param[SB_TreeAlign] && m_pTV->m_bSetRoot) {
		if (m_pTV->Create()) {
			m_pTV->SetRoot();
		}
	}
...
void CteShellBrowser::Show(BOOL bShow)
{
	if (bShow) {
		...
		if (!g_nLockUpdate) { 
			...
			if (m_nUnload == 9) {
				m_nUnload = 0;
				Navigate1(m_pFolderItem, SBSP_SAMEBROWSER | SBSP_WRITENOHISTORY, NULL, NULL);
			}
		}
	}
	m_bVisible = bShow;
...
void CteTabs::Close(BOOL bForce)
{
	if (CanClose(this) || bForce) {
		...
		SetWindowLongPtr(m_hwndStatic, GWLP_WNDPROC, (LONG_PTR)g_DefBTProc2);
...
BOOL CteTabs::Create()
{
	m_hwndStatic = CreateWindowEx(
		0, WC_BUTTON, NULL, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | BS_CHECKBOX,
		CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, 
		g_hwndMain, (HMENU)0, hInst, NULL); 
	g_DefBTProc2 = (WNDPROC)SetWindowLongPtr(m_hwndStatic, GWLP_WNDPROC, (LONG_PTR)TEBTProc2);
	BringWindowToTop(m_hwndStatic);

	m_hwndButton = CreateWindowEx(
		0, WC_BUTTON, NULL, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | BS_OWNERDRAW | BS_CHECKBOX,
		CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, 
...
STDMETHODIMP CteTreeView::Invoke(DISPID dispIdMember, REFIID riid, LCID lcid, WORD wFlags, DISPPARAMS *pDispParams, VARIANT *pVarResult, EXCEPINFO *pExcepInfo, UINT *puArgErr)
{
	...
	switch(dispIdMember) {
		...
		//Close
		case 0x10000004:
			m_pFV->m_param[SB_TreeAlign] = 0;//todo
			ArrangeWindow();
			return S_OK;
		...
		case 0x10000008:
			if (nArg >= 0) {
				m_pFV->m_param[SB_TreeAlign] = (GetIntFromVariant(&pDispParams->rgvarg[nArg]) & 6);
				if (!m_pNameSpaceTreeControl && !m_pShellNameSpace) {
					if (m_pFV->m_param[SB_TreeAlign]) {
						if (Create()) {
							if (m_bSetRoot) {
			...
			if (pVarResult) {
				pVarResult->lVal = m_pFV->m_param[SB_TreeAlign] ? 3 : 1;
				pVarResult->vt = VT_I4;
			}
			return S_OK;
		//Visible
...
VOID CteTreeView::SetChildren()
{
	if (m_pShellNameSpace && !(m_pFV->m_param[SB_EnumFlags] & SHCONTF_NONFOLDERS)) {
		...
		item.hItem = TreeView_GetSelection(m_hwndTV);
		if (item.hItem) {
			TreeView_EnsureVisible(m_hwndTV, item.hItem);
			IDispatch *pItem;
...
{
	HRESULT hr = teGetDispId(methodFI, sizeof(methodFI), g_maps[MAP_FI], *rgszNames, rgDispId, true); 
	if SUCCEEDED(hr) {
		return S_OK;
	}
	if (GetFolderItem()) {
		...
		if (nArg >= 0) {
			...
			switch(dispIdMember) {
				case 6000://ILCreateFromPath
					FolderItem *pid;
					IUnknown *punk;
					if (!FindUnknown(&pDispParams->rgvarg[nArg], &punk) || FAILED(punk->QueryInterface(IID_PPV_ARGS(&pid)))) {
						pid = new CteFolderItem(&pDispParams->rgvarg[nArg]);
					}
					teSetObject(pVarResult, pid);
					pid->Release();
					break;
							...
							if SUCCEEDED(lpfnPSPropertyKeyFromStringEx(v.bstrVal, &propKey)) {
								if (lpfnPSGetPropertyDescription) {
									...
									if SUCCEEDED(lpfnPSGetPropertyDescription(propKey, IID_PPV_ARGS(&pdesc))) {
										pdesc->FormatForDisplay(*((PROPVARIANT *)&pDispParams->rgvarg[nArg - 1]), (PROPDESC_FORMAT_FLAGS)GetIntFromVariant(&pDispParams->rgvarg[nArg - 2]), &pszResult);
										pdesc->Release();
									}
								}
#ifdef _2000XP
								else {
									pszResult = new WCHAR[MAX_PROP];
									g_pPUI->FormatForDisplay(propKey.fmtid, propKey.pid, (PROPVARIANT *)&pDispParams->rgvarg[nArg - 1], GetIntFromVariant(&pDispParams->rgvarg[nArg - 2]), pszResult, MAX_PROP);
									dispIdMember = 5;
								}
#endif
							}
					...
					if (v.bstrVal && lpfnPSPropertyKeyFromStringEx) {
						...
						if SUCCEEDED(lpfnPSPropertyKeyFromStringEx(v.bstrVal, &propKey)) {
							if (lpfnPSGetPropertyDescription) {
								IPropertyDescription *pdesc;
								if SUCCEEDED(lpfnPSGetPropertyDescription(propKey, IID_PPV_ARGS(&pdesc))) {
									pdesc->GetDisplayName(&pszResult);
									pdesc->Release();
								}
							}
#ifdef _2000XP
								...
								pszResult = new WCHAR[MAX_PROP];
								g_pPUI->GetDisplayName(propKey.fmtid, propKey.pid, PUIFNF_DEFAULT, pszResult, MAX_PROP);
								dispIdMember = 5;
							}
#endif
						}
