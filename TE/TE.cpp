...
{
	if (pObj) {
		if SUCCEEDED(static_cast<IUnknown *>(pObj)->QueryInterface(IID_PPV_ARGS(&pv->pdispVal))) {
			pv->vt = VT_DISPATCH;
			return true;
		}
		if SUCCEEDED(static_cast<IUnknown *>(pObj)->QueryInterface(IID_PPV_ARGS(&pv->punkVal))) {
			pv->vt = VT_UNKNOWN;
			return true;
		}
	}
